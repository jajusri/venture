# VENTURE controlled-pilot, two-company / two-phone physical enrollment orchestration.
#
...
# existing tools (`venture-services.ps1`, `venture-android.ps1`, `pilot-provision.ts`) in the right
# order and move non-secret bytes between them.
#
# Usage:
#   .\scripts\venture-controlled-pilot.ps1 -Doctor
#   .\scripts\venture-controlled-pilot.ps1 -Bootstrap
#   .\scripts\venture-controlled-pilot.ps1 -Start
#   .\scripts\venture-controlled-pilot.ps1 -Enroll
#   .\scripts\venture-controlled-pilot.ps1 -Verify
#   .\scripts\venture-controlled-pilot.ps1 -Status
#   .\scripts\venture-controlled-pilot.ps1 -RunTransport
#   .\scripts\venture-controlled-pilot.ps1 -RunLifecycle
#   .\scripts\venture-controlled-pilot.ps1 -Stop
#   .\scripts\venture-controlled-pilot.ps1 -RunAll
#
...
$Script:DevDebugPackage = 'com.jajusri.venture.dev.debug'
$Script:PilotReceiver = "$DevDebugPackage/com.jajusri.venture.pilotharness.PilotEnrollmentReceiver"
...
$Script:TransportReceiver = "$DevDebugPackage/com.jajusri.venture.pilotharness.PilotTransportReceiver"
...
function Import-DotEnv {
    ...
    if (-not (Test-Path $envFile)) {
        Write-Warn "backend\.env not found -- copy backend\.env.example to backend\.env and set VENTURE_TRUST_DATABASE_URL first."
        return
    }
...
$Script:TestCompanies = @(
    [ordered]@{ Index = 1; Slug = 'pilot-test-1'; DisplayName = 'VENTURE Test 1 Company'; Actor = 'pilot-test-1-actor'; VerificationId = 'pilot-test-1-verified' },
    [ordered]@{ Index = 2; Slug = 'pilot-test-2'; DisplayName = 'VENTURE Test 2 Company'; Actor = 'pilot-test-2-actor'; VerificationId = 'pilot-test-2-verified' }
)

function Get-HostTrustPort {
    # Where Trust is actually listening on the HOST right now -- normally == $TrustPort, but may be
    # session-overridden (VENTURE_TRUST_PORT in backend/.env) to avoid colliding with an unrelated
    ...
    if ($env:VENTURE_TRUST_PORT) { return $env:VENTURE_TRUST_PORT }
    return $TrustPort
}
...
function Invoke-BackendCli {
    ...
    try {
        ...
        # npm's own banner lines ("> @venture/backend...", "> tsx ...") precede the JSON payload --
...
function Invoke-PilotDoctor {
    ...
    & (Join-Path $RepoRoot 'scripts\venture-services.ps1') -Doctor
    ...
    $localProps = Join-Path $RepoRoot 'apps\venture_android\local.properties'
    ...
    $gradlew = Join-Path $RepoRoot 'apps\venture_android\gradlew.bat'
...
function Invoke-PilotStart {
    ...
    & (Join-Path $RepoRoot 'scripts\venture-services.ps1') -Start -Verify
...
function Invoke-PilotEnrollOne {
    ...
    # `venture-android.ps1`'s own internal adb calls print raw text that isn't captured there either;
    ...
    & (Join-Path $RepoRoot 'scripts\venture-android.ps1') -DeviceSerial $Serial -Variant DevDebug -Install -Launch -Verify | Out-Host
    if ($LASTEXITCODE -ne 0) { Write-ErrorAndExit "devDebug install/launch/verify failed on $Serial. Run '.\scripts\venture-android.ps1 -DeviceSerial $Serial -Variant DevDebug -Build' first if no APK has been built yet for the current commit." }
...
function Invoke-PilotVerify {
    ...
    & (Join-Path $RepoRoot 'scripts\venture-services.ps1') -Verify
...
function Invoke-PilotRunLifecycle {
    ...
    $serials = Resolve-PhoneSerials
    & (Join-Path $RepoRoot 'scripts\venture-services.ps1') -Verify | Out-Host
    ...
    $bindAonB = Invoke-PilotBindCounterparty -Serial $serials.A -Label 'Phone A' -PeerDisplayName 'VENTURE Test 2 Company' -PeerCredential $identityB.credential
...
function Invoke-PilotRunTransport {
    ...
    # Trust is actually listening now (VENTURE_TRUST_PORT, when overridden for this session -- see
    ...
    $bindAonB = Invoke-PilotBindCounterparty -Serial $serials.A -Label 'Phone A' -PeerDisplayName 'VENTURE Test 2 Company' -PeerCredential $identityB.credential
    $bindBonA = Invoke-PilotBindCounterparty -Serial $serials.B -Label 'Phone B' -PeerDisplayName 'VENTURE Test 1 Company' -PeerCredential $identityA.credential
...
function Invoke-PilotStop {
    Write-Section 'Stop'
    & (Join-Path $RepoRoot 'scripts\venture-services.ps1') -Stop
}
...
if (-not ($Doctor -or $Bootstrap -or $Start -or $Enroll -or $Verify -or $Status -or $Stop -or $RunTransport -or $RunLifecycle -or $RunAll)) {
    ...
    Write-Host '  .\scripts\venture-controlled-pilot.ps1 -Doctor'
    Write-Host '  .\scripts\venture-controlled-pilot.ps1 -RunAll'
    exit 0
}
