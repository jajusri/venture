# VENTURE combined operator flow. Orchestrates the already-proven platform-specific
# tools (venture-doctor.ps1, venture-desktop.ps1, venture-android.ps1) -- does not
# duplicate their logic.
#
# SAFETY DEFAULT: the Desktop portion never touches the user's live, currently-running
# Desktop/Connector session -- it only builds and reports status. venture-desktop.ps1's
# own -Launch/-Restart modes stop any currently-running VENTURE desktop/connector
...
#
# Usage:
#   .\scripts\venture-install.ps1 -Doctor
#   .\scripts\venture-install.ps1 -DesktopOnly -Build
#   .\scripts\venture-install.ps1 -DesktopOnly -DesktopInstall                          # dry run: reports current vs. installed version
#   .\scripts\venture-install.ps1 -DesktopOnly -DesktopInstall -DesktopInstallConfirm   # actually launches the installer (real UAC prompt)
#   .\scripts\venture-install.ps1 -DesktopOnly -DesktopInstalledVerify                  # read-only checks against whatever is currently installed
#   .\scripts\venture-install.ps1 -AndroidOnly -Build -Install -Launch -Verify -DeviceSerial <SERIAL> -Variant DevDebug     # development/pilot
#   .\scripts\venture-install.ps1 -AndroidOnly -Build -Install -Launch -Verify -DeviceSerial <SERIAL> -Variant ProdRelease  # production/release
#   .\scripts\venture-install.ps1 -Build -Install -Launch -Verify -DeviceSerial <SERIAL> -Variant DevDebug   # both platforms
#
...
function Get-LatestDesktopInstaller {
    ...
    Get-ChildItem -Path $artifactsRoot -Recurse -Filter 'VentureDesktop-*-x64-setup.exe' -ErrorAction SilentlyContinue |
...
function Get-InstalledDesktopVersion {
    ...
    # electron-builder's NSIS DisplayName embeds the version ("Venture Desktop 0.4.20"), so match by
    ...
    foreach ($root in $roots) {
        ...
        foreach ($entry in $candidates) {
            ...
            if (-not $hasDisplayName -or $entry.DisplayName -notlike 'Venture Desktop*') { continue }
            ...
            if (-not $installLocation -and ($entry.PSObject.Properties.Name -contains 'UninstallString')) {
                ...
                # path (e.g. `"...\Uninstall Venture Desktop.exe" /allusers`) -- extract only the quoted
...
function Invoke-DesktopInstallStage {
    ...
    $versionMatch = [regex]::Match($installer.Name, 'VentureDesktop-(?<v>[\d.]+)-x64-setup\.exe')
    ...
    if ($installed -and $installerVersion -and $installed.Version -eq $installerVersion -and $installed.InstallLocation) {
        $installedExe = Join-Path $installed.InstallLocation 'Venture Desktop.exe'
    ...
    $fromLabel = if ($installed) { "$($installed.Version) (installed files dated $((Get-Item (Join-Path $installed.InstallLocation 'Venture Desktop.exe') -ErrorAction SilentlyContinue).LastWriteTime))" } else { '(not installed)' }
...
function Invoke-DesktopInstalledVerifyStage {
    ...
    if (-not $installed -or -not $installed.InstallLocation) {
        Add-Result 'Desktop Installed Verify' 'ENVIRONMENT_MISSING' 'Venture Desktop is not registered as installed'
        return
    }
    $exePath = Join-Path $installed.InstallLocation 'Venture Desktop.exe'
    if (-not (Test-Path $exePath)) {
        Add-Result 'Desktop Installed Verify' 'FAIL' "registered install location $($installed.InstallLocation) has no Venture Desktop.exe"
        return
    }
    $procs = @(Get-Process -Name 'Venture Desktop' -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $exePath })
    # desktop-config.json holds Connector/runtime settings, not the selected company -- that lives
    # behind the Connector's own session-status API/logs (see venture-desktop.ps1 -Status), not a
    # static file this read-only check can safely parse. Only report config presence/validity here.
    $configPath = Join-Path $env:APPDATA '@venture\desktop\desktop-config.json'
...
if ($Doctor) {
    Invoke-Stage 'Doctor' { & (Join-Path $RepoRoot 'scripts\venture-doctor.ps1') }
}

if ($doDesktop) {
    if ($Build) {
        Invoke-Stage 'Desktop Build' { & (Join-Path $RepoRoot 'scripts\venture-desktop.ps1') -Build }
    }
    if ($DesktopLaunch -and $Launch) {
        Invoke-Stage 'Desktop Launch+Verify (opt-in, stops live instance first)' {
            & (Join-Path $RepoRoot 'scripts\venture-desktop.ps1') -Launch -Verify
        }
    }
    elseif ($Launch) {
    ...
    if (-not $Build -and -not ($DesktopLaunch -and $Launch)) {
        Invoke-Stage 'Desktop Status (read-only)' { & (Join-Path $RepoRoot 'scripts\venture-desktop.ps1') -Status }
    }
...
if ($doAndroid) {
    ...
    elseif (-not $Variant) {
        ...
        if ($deviceLocked) {
            ...
            # venture-android.ps1's params positionally instead of by name (reproduced live: the device
            ...
            Invoke-Stage "Android ($Variant @ $DeviceSerial)" {
                & (Join-Path $RepoRoot 'scripts\venture-android.ps1') @androidArgs
            }
        }
    }
}
