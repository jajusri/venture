# Canonical VENTURE Android developer deploy/status tool.
# Usage:
#   .\scripts\venture-android.ps1 -Status -DeviceSerial <SERIAL>
#   .\scripts\venture-android.ps1 -DeviceSerial <SERIAL> -Variant ProdDebug -Build -Install -Launch -Verify
...
$Script:AndroidDir = Join-Path $RepoRoot 'apps\venture_android'
...
$Script:LauncherLogDir = Join-Path $env:TEMP 'venture-android-launcher'
...
function Resolve-VariantConfig {
    ...
    $package = switch ($Name) {
        'ProdDebug' { 'com.jajusri.venture.debug' }
        'DevDebug' { 'com.jajusri.venture.dev.debug' }
        'ProdRelease' { 'com.jajusri.venture' }
        'DevRelease' { 'com.jajusri.venture.dev' }
    ...
    return [ordered]@{
        ...
        Package = $package
        LaunchActivity = "$package/com.jajusri.venture.app.MainActivity"
    }
}
...
if (-not ($Status -or $Build -or $Install -or $Launch -or $Verify)) {
    ...
    Write-Step '  .\scripts\venture-android.ps1 -Status -DeviceSerial <SERIAL>'
    Write-Step '  .\scripts\venture-android.ps1 -DeviceSerial <SERIAL> -Variant ProdDebug -Build -Install -Launch -Verify'
    exit 0
}
