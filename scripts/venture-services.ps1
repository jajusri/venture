# VENTURE Trust + Relay local controlled-pilot service runner.
#
...
#
# Usage:
#   .\scripts\venture-services.ps1 -Doctor
#   .\scripts\venture-services.ps1 -Status
#   .\scripts\venture-services.ps1 -StartTrust
#   .\scripts\venture-services.ps1 -StartRelay
#   .\scripts\venture-services.ps1 -Start              # both, Trust first
#   .\scripts\venture-services.ps1 -Start -Verify
#   .\scripts\venture-services.ps1 -Verify
#   .\scripts\venture-services.ps1 -Stop               # only processes THIS tool started
#
...
$StateDir = Join-Path $env:TEMP 'venture-services'
...
$TrustPort = [int](Get-EnvOrDefault 'VENTURE_TRUST_PORT' '8080')
$RelayPort = [int](Get-EnvOrDefault 'VENTURE_RELAY_PORT' '8082')
$TrustHost = Get-EnvOrDefault 'VENTURE_TRUST_HOST' '127.0.0.1'
$RelayHostName = Get-EnvOrDefault 'VENTURE_RELAY_HOST' '127.0.0.1'
$TrustDbUrl = Get-EnvOrDefault 'VENTURE_TRUST_DATABASE_URL' ''
$RelayDbUrl = Get-EnvOrDefault 'VENTURE_RELAY_DATABASE_URL' $TrustDbUrl
...
function Start-VentureService {
    ...
    if (Test-PortListening -TargetHost $TargetHost -Port $Port) {
        ...
        Write-Host "      Free the port, or set VENTURE_$($Name.ToUpper())_PORT to a different value, then retry."
        return
    }
...
function Show-Doctor {
    Write-Host '=== VENTURE Trust + Relay doctor (read-only) ==='
    ...
    Write-Host "VENTURE_TRUST_DATABASE_URL: $(Redact-ConnectionString $TrustDbUrl)"
    Write-Host "VENTURE_RELAY_DATABASE_URL: $(Redact-ConnectionString $RelayDbUrl) $(if (-not (Get-EnvOrDefault 'VENTURE_RELAY_DATABASE_URL' '')) { '(defaults to Trust''s URL)' })"
    ...
    if ([string]::IsNullOrWhiteSpace($TrustDbUrl)) {
        Write-Host 'FAIL: VENTURE_TRUST_DATABASE_URL is not set -- Trust cannot start without it. See backend/.env.example.' -ForegroundColor Red
    } else {
        $target = Get-DatabaseHostPort -ConnectionString $TrustDbUrl
        if (-not $target) {
            Write-Host 'WARN: could not parse host/port out of VENTURE_TRUST_DATABASE_URL to test reachability.' -ForegroundColor Yellow
            ...
            Write-Host "FAIL: nothing reachable at $($target.HostName):$($target.Port). Start PostgreSQL, or point VENTURE_TRUST_DATABASE_URL at a reachable instance." -ForegroundColor Red
        }
    }
...
function Show-Status {
    Write-Host '=== VENTURE Trust + Relay status ==='
    foreach ($svc in @(
...
if (-not ($Doctor -or $Status -or $StartTrust -or $StartRelay -or $Start -or $Verify -or $Stop)) {
    ...
    Write-Host '  .\scripts\venture-services.ps1 -Doctor'
    Write-Host '  .\scripts\venture-services.ps1 -Start -Verify'
    Write-Host '  .\scripts\venture-services.ps1 -Status'
    Write-Host '  .\scripts\venture-services.ps1 -Stop'
    exit 0
}

if ($Doctor) { Show-Doctor }
if ($StartTrust -or $Start) { Start-VentureService -Name 'trust' -NpmScript 'dev' -Port $TrustPort -TargetHost $TrustHost }
if ($StartRelay -or $Start) { Start-VentureService -Name 'relay' -NpmScript 'dev:relay' -Port $RelayPort -TargetHost $RelayHostName }
...
if ($Stop) {
    Stop-VentureService -Name 'trust'
    Stop-VentureService -Name 'relay'
}
exit 0
