# Canonical VENTURE Desktop developer launcher (read-only Status, bounded Build/Launch/Verify/Restart).
# Usage:
...
$Script:DesktopDir = Join-Path $RepoRoot 'apps\venture_desktop'
...
$Script:PackageName = '@venture/desktop'
$Script:AppId = 'com.venture.desktop'
$Script:ProductName = 'Venture Desktop'
$Script:WindowTitle = 'Business OS Tally Connector'
$Script:UserDataDir = Join-Path $env:APPDATA '@venture\desktop'
$Script:DesktopConfigPath = Join-Path $UserDataDir 'desktop-config.json'
$Script:DesktopLogPath = Join-Path $UserDataDir 'logs\venture-desktop.log'
$Script:LauncherLogDir = Join-Path $env:TEMP 'venture-desktop-launcher'
...
function Get-VentureDesktopProcesses {
    $results = @()
    ...
    foreach ($proc in $cim) {
        $results += [pscustomobject]@{
            ...
            Kind = if ($proc.Name -eq 'Venture Desktop.exe') { 'packaged' } else { 'dev-electron' }
        }
    }
    return $results
}

function Get-VentureConnectorProcesses {
    $results = @()
    $cim = @(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
        Where-Object {
            $_.CommandLine -and
            $_.CommandLine -match 'venture_connector[\\/]+dist[\\/]+main\.js' -and
...
function Read-DesktopStartupLogTail {
    ...
    foreach ($line in $lines) {
        if ($line -match 'uncaughtException|\[venture-desktop:startup\] uncaughtException|Fatal bootstrap error') {
            $fatal = $true
            break
        }
    }
...
function Invoke-VentureBuild {
    Write-Step '[3/6] Build - npm run build (apps/venture_desktop)'
...
function Stop-VentureDesktopProcesses {
    param([string]$Reason)
    $desktop = @(Get-VentureDesktopProcesses)
    $connector = @(Get-VentureConnectorProcesses)
    if ($desktop.Count -eq 0 -and $connector.Count -eq 0) {
        Write-Step "  no VENTURE desktop/connector processes to stop ($Reason)"
        return
    }
    ...
    while ((Get-Date) -lt $deadline) {
        if (@(Get-VentureDesktopProcesses).Count -eq 0 -and @(Get-VentureConnectorProcesses).Count -eq 0) {
    ...
    foreach ($proc in (Get-VentureDesktopProcesses + Get-VentureConnectorProcesses)) {
        try {
...
function Invoke-VentureLaunch {
    ...
    if (-not (Test-Path $ElectronExe)) {
        Write-Fail "Electron binary missing at $ElectronExe - run npm install in apps/venture_desktop"
    }
    if (-not (Test-Path $MainOutput)) {
        Write-Fail "Build output missing at $MainOutput - run with -Build first"
    }

    $running = @(Get-VentureDesktopProcesses)
...
function Invoke-VentureVerify {
    ...
    if (-not $observedPid) {
        $log = Read-DesktopStartupLogTail
        Write-Fail "No VENTURE desktop process detected within ${StartupTimeoutSec}s. Desktop log fatal=$($log.Fatal). Launcher log: $LauncherLogFile"
    }

    Write-Step ("  desktop process alive: PID {0}" -f $observedPid)
    Start-Sleep -Seconds $VerifyAliveSec
    $still = Get-VentureDesktopProcesses | Where-Object { $_.ProcessId -eq $observedPid }
    if (-not $still) {
        $log = Read-DesktopStartupLogTail
...
function Show-VentureStatus {
    Write-Step '=== VENTURE Desktop Status ==='
    ...
    Write-Step 'Build system: npm + TypeScript + Electron (apps/venture_desktop/package.json)'
...
if (-not ($Status -or $Build -or $Launch -or $Verify -or $Restart)) {
    ...
    Write-Step '  .\scripts\venture-desktop.ps1 -Status'
    Write-Step '  .\scripts\venture-desktop.ps1 -Build -Launch -Verify'
    Write-Step '  .\scripts\venture-desktop.ps1 -Restart -Verify'
    exit 0
}
...
if ($Restart) {
    ...
    Stop-VentureDesktopProcesses -Reason 'restart'
    $Launch = $true
}
...
if ($Launch) {
    Invoke-VentureLaunch
}

if ($Verify) {
    Invoke-VentureVerify
}
elseif ($Launch) {
