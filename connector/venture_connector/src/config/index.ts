...
export function loadConfig(overrides: Partial<ConnectorConfig> = {}): ConnectorConfig {
  ...
  const config: ConnectorConfig = {
    ...
    tallyRequestAuditMaxFiles: parseBoundedInt(
      process.env.VENTURE_TALLY_REQUEST_AUDIT_MAX_FILES,
    ...
    voucherSyncFailureAuditEnabled: parseBoolean(
      process.env.VENTURE_VOUCHER_SYNC_FAILURE_AUDIT,
      defaultConfig.voucherSyncFailureAuditEnabled,
    ),
    voucherSyncFailureAuditPath:
      process.env.VENTURE_VOUCHER_SYNC_FAILURE_AUDIT_PATH ?? defaultConfig.voucherSyncFailureAuditPath,
    databasePath: process.env.VENTURE_DATABASE_PATH ?? defaultConfig.databasePath,
    gracefulShutdownMs: parsePositiveInt(
      process.env.VENTURE_SHUTDOWN_MS,
    ...
    sessionTtlMs: parsePositiveInt(
      process.env.VENTURE_SESSION_TTL_MS,
      defaultConfig.sessionTtlMs,
    ),
    syncRunHistoryMaxCount: parseBoundedInt(
      process.env.VENTURE_SYNC_RUN_HISTORY_MAX_COUNT,
    ...
    syncRunHistoryMaxAgeDays: parseBoundedInt(
      process.env.VENTURE_SYNC_RUN_HISTORY_MAX_AGE_DAYS,
      defaultConfig.syncRunHistoryMaxAgeDays,
      SYNC_RUN_HISTORY_MAX_AGE_DAYS_MIN,
      SYNC_RUN_HISTORY_MAX_AGE_DAYS_LIMIT,
      'sync run history max age days',
    ),
    startupCorrelationId: process.env.VENTURE_STARTUP_CORRELATION_ID?.trim() || null,
    requireDeviceAuthForLan: parseBoolean(
      process.env.VENTURE_REQUIRE_DEVICE_AUTH_FOR_LAN,
      defaultConfig.requireDeviceAuthForLan,
    ),
    desktopControlToken: process.env.VENTURE_DESKTOP_CONTROL_TOKEN?.trim() || null,
    securePairingEnabled: parseBoolean(
      process.env.VENTURE_SECURE_PAIRING_ENABLED,
      defaultConfig.securePairingEnabled,
    ),
    secureTransportEnabled: parseBoolean(
      process.env.VENTURE_SECURE_TRANSPORT_ENABLED,
      defaultConfig.secureTransportEnabled,
    ),
    secureTransportPort: parsePort(
      process.env.VENTURE_SECURE_TRANSPORT_PORT,
      defaultConfig.secureTransportPort,
    ),
    transportIdentityDir: process.env.VENTURE_TRANSPORT_IDENTITY_DIR ?? defaultConfig.transportIdentityDir,
    secureLanRouteProtectionEnabled: parseBoolean(
      process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED,
      defaultConfig.secureLanRouteProtectionEnabled,
    ),
    privateStorageExpectedVaultId: process.env.VENTURE_PRIVATE_STORAGE_VAULT_ID?.trim() || null,
    privateStorageMarkerPath: process.env.VENTURE_PRIVATE_STORAGE_MARKER_PATH?.trim() || null,
    ...overrides,
  };
  ...
  if (
    ...
    throw new Error(
      'Non-loopback connector bind requires VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED=true in production.',
    );
  }
