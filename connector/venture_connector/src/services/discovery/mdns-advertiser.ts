...
/** _venture._tcp.local — bonjour-service appends the `_tcp` suffix itself from `type`. */
export const VENTURE_MDNS_SERVICE_TYPE = 'venture';
...
/**
 * Advertises the Connector over standards-based mDNS/DNS-SD (_venture._tcp.local) so Android can
...
export class MdnsAdvertiser implements ServiceLifecycle {
  ...
  getStatus(): ServiceStatus {
    return {
      ...
      ready: this.running,
      message: this.running ? 'Advertising _venture._tcp.local' : 'Not advertising',
    };
  }
  ...
  private buildOptions(): MdnsPublishOptions {
    ...
    return {
      name: identity.connectorName,
      type: VENTURE_MDNS_SERVICE_TYPE,
