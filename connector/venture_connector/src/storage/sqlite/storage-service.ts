...
export class SqliteStorageService implements LocalDatabaseService {
  ...
  async start(): Promise<void> {
    ...
    const databasePath = path.join(this.config.databasePath, 'venture-ledger.db');
  ...
  createBackup(targetDir: string): { ok: boolean; backupPath: string | null; message: string } {
    ...
    try {
      ...
      const source = path.join(this.config.databasePath, 'venture-ledger.db');
      ...
      const backupFileName = `venture-ledger-${Date.now()}.db`;
