...
afterEach(() => {
  for (const temporaryRoot of temporaryRoots.splice(0)) {
    if (path.basename(temporaryRoot).startsWith('venture-voucher-discovery-')) {
...
function executionSetup(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-voucher-discovery-'));
...
describe('executable Voucher discovery harness', () => {
  it('builds the BASELINE-04 read-only embedded Voucher collection request', () => {
    ...
    expect(request.xml).toMatch(
      /<HEADER>[\s\S]*<TYPE>Collection<\/TYPE>[\s\S]*<ID>Venture Voucher Discovery<\/ID>[\s\S]*<\/HEADER>/,
    );
    expect(request.xml).toContain(
      `<COLLECTION NAME="${VOUCHER_DISCOVERY_COLLECTION_NAME}" ISMODIFY="No" ISFIXED="No" ISINITIALIZE="Yes">`,
    );
    expect(request.xml).toMatch(
      /<COLLECTION NAME="Venture Voucher Discovery"[\s\S]*?<TYPE>Voucher<\/TYPE>[\s\S]*?<\/COLLECTION>/,
    );
