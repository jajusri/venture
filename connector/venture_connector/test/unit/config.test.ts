...
describe('loadConfig', () => {
  it('loads defaults', () => {
    ...
    expect(config.connectorVersion).toBe('0.4.6');
...
describe('secureLanRouteProtectionEnabled', () => {
  const ORIGINAL_ENV = process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED;

  afterEach(() => {
    if (ORIGINAL_ENV === undefined) {
      delete process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED;
    } else {
      process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED = ORIGINAL_ENV;
    }
  });
  ...
  it('parses an explicit "true" env value', () => {
    process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED = 'true';
    const config = loadConfig({ env: 'test' });
    expect(config.secureLanRouteProtectionEnabled).toBe(true);
  });

  it('parses an explicit "false" env value', () => {
    process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED = 'false';
    const config = loadConfig({ env: 'test' });
    expect(config.secureLanRouteProtectionEnabled).toBe(false);
  });

  it('rejects a malformed env value rather than silently defaulting', () => {
    process.env.VENTURE_SECURE_LAN_ROUTE_PROTECTION_ENABLED = 'yes-please';
