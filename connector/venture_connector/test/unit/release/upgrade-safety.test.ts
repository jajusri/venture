...
describe('sqlite migration safety integration', () => {
  ...
  it('rejects database with future schema version', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-schema-future-'));
    tempDirs.push(dir);
    const dbPath = path.join(dir, 'venture-ledger.db');
  ...
  it('reopens idempotently after successful migration baseline', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-schema-current-'));
    tempDirs.push(dir);
    const dbPath = path.join(dir, 'venture-ledger.db');
