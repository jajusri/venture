...
describe('ConnectorIdentityRepository', () => {
  ...
  it('prefers a Desktop-supplied VENTURE_CONNECTOR_ID over generating one', async () => {
    const { storage } = await createTestSqliteStorage();
    const repo = new ConnectorIdentityRepository(
      () => storage.getBundle().database,
      { VENTURE_CONNECTOR_ID: 'desktop-issued-id-123' },
    );
  ...
  it('uses VENTURE_CONNECTOR_NAME when present, otherwise falls back to a friendly default', async () => {
    const { storage } = await createTestSqliteStorage();
    const named = new ConnectorIdentityRepository(
      () => storage.getBundle().database,
      { VENTURE_CONNECTOR_NAME: 'Front Desk PC' },
    );
