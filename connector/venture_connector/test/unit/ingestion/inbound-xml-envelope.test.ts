...
describe('InboundXmlEnvelopeService', () => {
  ...
  describe('filesystem ingestion', () => {
    it('accepts a trusted internal file path', async () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-inbound-'));
      tempDirs.push(dir);
    ...
    it('rejects missing files', async () => {
      const result = await makeService().acceptFile({
        sourceType: InboundXmlSourceType.TrustedInternalFile,
        filePath: path.join(os.tmpdir(), 'missing-venture-import.xml'),
        targetCompanyName: 'Pilot Co',
      });
    ...
    it('rejects directories', async () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-inbound-dir-'));
      tempDirs.push(dir);
      const result = await makeService().acceptFile({
        sourceType: InboundXmlSourceType.TrustedInternalFile,
        filePath: dir,
    ...
    it('rejects files outside watched-folder root', async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-root-'));
      const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-outside-'));
