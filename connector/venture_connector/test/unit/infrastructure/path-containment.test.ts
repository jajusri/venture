...
describe('path containment', () => {
  it('allows contained child directories', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-contain-root-'));
  ...
  it('rejects symlink segments beneath the root', () => {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-contain-root-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-contain-outside-'));
    const linkPath = path.join(root, 'linked-backups');
    fs.symlinkSync(outside, linkPath, 'dir');
    expect(() =>
      assertWriteTargetContained({
        candidatePath: path.join(linkPath, 'venture-ledger-1.db'),
        rootPath: root,
      }),
  ...
  it('rejects non-existing targets beneath a linked parent', () => {
    ...
    expect(() =>
      assertWriteTargetContained({
        candidatePath: path.join(root, 'linked', 'new', 'venture-ledger-1.db'),
        rootPath: root,
      }),
  ...
  it('blocks protected write targets', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-contain-root-'));
    const liveDb = path.join(root, 'venture-ledger.db');
