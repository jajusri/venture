...
describe('sync run history retention configuration (B2c)', () => {
  ...
  it('accepts valid environment overrides', () => {
    const previousCount = process.env.VENTURE_SYNC_RUN_HISTORY_MAX_COUNT;
    const previousAge = process.env.VENTURE_SYNC_RUN_HISTORY_MAX_AGE_DAYS;
    process.env.VENTURE_SYNC_RUN_HISTORY_MAX_COUNT = '50';
    process.env.VENTURE_SYNC_RUN_HISTORY_MAX_AGE_DAYS = '30';
    try {
      const config = loadConfig({ env: 'test' });
      ...
      if (previousCount === undefined) {
        delete process.env.VENTURE_SYNC_RUN_HISTORY_MAX_COUNT;
      } else {
        process.env.VENTURE_SYNC_RUN_HISTORY_MAX_COUNT = previousCount;
      }
      if (previousAge === undefined) {
        delete process.env.VENTURE_SYNC_RUN_HISTORY_MAX_AGE_DAYS;
      } else {
        process.env.VENTURE_SYNC_RUN_HISTORY_MAX_AGE_DAYS = previousAge;
      }
    }
  });
