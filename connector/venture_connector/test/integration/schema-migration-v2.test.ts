...
describe('schema migration v1 to v2', () => {
  it('preserves ledger rows and assigns resource_kind=ledgers to historical sync runs', () => {
    const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-migrate-v2-'));
  ...
  it('upgrades partial stock_items tables missing identity columns', () => {
    const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-migrate-v3-partial-'));
    tempDirs.push(basePath);
    const databasePath = path.join(basePath, 'venture-ledger.db');
  ...
  it('is idempotent when opened twice at current schema version', () => {
    const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-migrate-idempotent-'));
    tempDirs.push(basePath);
    const databasePath = path.join(basePath, 'venture-ledger.db');
  ...
  it('adds predecessor_sync_run_id on v3 to v4 migration with null defaults', () => {
    const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-migrate-v4-'));
    tempDirs.push(basePath);
    const databasePath = path.join(basePath, 'venture-ledger.db');
