...
/**
 * Resolve VENTURE_TALLY_TEST_COMPANY for harness requests.
...
export function resolveRequestedCompanyContext(rawEnv: string | undefined): RequestedCompanyContext {
  ...
  if (!trimmed) {
    throw new Error(
      'Set VENTURE_TALLY_TEST_COMPANY to the dedicated test company display name before running live scenarios.',
    );
  }
  ...
  if (!normalizedName) {
    throw new Error('VENTURE_TALLY_TEST_COMPANY normalized to an empty company name.');
  }
