...
const SYNTHETIC_RENAMED_LEDGER = 'VENTURE TEST LEDGER A RENAMED';
const SYNTHETIC_ORIGINAL_LEDGER = 'VENTURE TEST LEDGER A';
const TARGET_COMPANY_SLUG = 'venture-test-01';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const slugOriginalMatches = postFirstItems.filter((item) => item.id === 'name:venture-test-ledger-a');
    ...
    scenarios.push(
      await runScenario('Phase D — API GUID-ID lookup', async () => {
        ...
        const search = await httpJson<{ items: unknown[]; pagination: { totalItems: number } }>(
          baseUrl,
          `/ledgers?query=${encodeURIComponent('VENTURE TEST')}&page=1&pageSize=10`,
        );
