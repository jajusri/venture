...
export interface ConnectorIdentity {
   ...
   * configuration (connector-identity.json) and passed it via BUDCOM_CONNECTOR_ID. This is the
...
export class ConnectorIdentityRepository {
  ...
  getOrCreateIdentity(): ConnectorIdentity {
    ...
    const connectorName = this.env.BUDCOM_CONNECTOR_NAME?.trim() || safeHostname();
    const suppliedId = this.env.BUDCOM_CONNECTOR_ID?.trim();
    if (suppliedId) {
...
function safeHostname(): string {
  try {
    return os.hostname() || 'BUDCOM Connector';
  } catch {
    return 'BUDCOM Connector';
  }
}
