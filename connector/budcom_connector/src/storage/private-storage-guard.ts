...
export function assertPrivateStorageVaultPresent(
  ...
  if (!fsImpl.existsSync(markerPath)) {
    throw new PrivateStorageUnavailableError(
      'Private BUDCOM storage is not connected: vault marker not found at the configured location.',
    );
  }
  ...
  try {
    ...
    throw new PrivateStorageUnavailableError('Private BUDCOM storage vault marker could not be read.');
  }

  if (typeof parsed.vaultId !== 'string' || parsed.vaultId !== expectedVaultId) {
    throw new PrivateStorageUnavailableError(
      'Connected removable storage does not match the configured private BUDCOM vault.',
    );
  }
}
