...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-transport-identity-test-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('ConnectorTransportIdentityService', () => {
  ...
  it('private key is never stored in SQLite', async () => {
    ...
    const dbPath = path.join(basePath, 'budcom-ledger.db');
    const { logger } = createCapturingLogger();
  ...
  it('the generated certificate contains no customer, company, or accounting information', async () => {
    ...
    expect(certificate.subject).toContain('budcom-connector.local');
  });
});
...
async function selfSignExpired(privateKeyPem: string): Promise<string> {
  ...
  const pems = await selfsigned.generate(
    [{ name: 'commonName', value: 'budcom-connector.local' }],
    {
      keyType: 'ec',
      curve: 'P-256',
      algorithm: 'sha256',
