...
describe('readStableInboundXmlFile', () => {
  ...
  it('rejects watched-folder files that keep changing until timeout', async () => {
    const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-watch-root-')));
    tempDirs.push(root);
  ...
  it('accepts watched-folder files after bounded stable-duration window', async () => {
    const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-watch-stable-')));
    tempDirs.push(root);
  ...
  it('leaves trusted-internal source bytes unchanged on disk', async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-file-stability-'));
    tempDirs.push(dir);
