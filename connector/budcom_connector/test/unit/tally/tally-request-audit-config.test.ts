...
describe('Tally request audit configuration (B2a)', () => {
  ...
  it('accepts valid environment overrides', () => {
    const previousBytes = process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES;
    const previousFiles = process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_FILES;
    process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES = String(128 * 1024);
    process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_FILES = '2';
    try {
      const config = loadConfig({ env: 'test' });
      ...
      if (previousBytes === undefined) {
        delete process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES;
      } else {
        process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES = previousBytes;
      }
      if (previousFiles === undefined) {
        delete process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_FILES;
      } else {
        process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_FILES = previousFiles;
      }
    }
  });
  ...
  it('rejects non-integer audit settings from environment variables', () => {
    const previousBytes = process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES;
    process.env.BUDCOM_TALLY_REQUEST_AUDIT_MAX_BYTES = 'abc';
    try {
  ...
  it('does not create audit file when disabled', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'budcom-audit-disabled-'));
