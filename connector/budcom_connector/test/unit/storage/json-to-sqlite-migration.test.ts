...
function createMigrationFixture(): {
  ...
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-json-migrate-'));
  ...
  return {
    ...
    legacyPath,
    backupPath,
    dbPath: path.join(basePath, 'budcom-ledger.db'),
  };
}

describe('JsonToSqliteMigrationService', () => {
  it('returns none when no legacy JSON exists', () => {
    const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'budcom-empty-migrate-'));
    tempDirs.push(basePath);
