# BUDCOM UI Design Decisions

**Status:** Active — locked visual direction, screen-level decisions pending further exploration
**Scope:** Presentation-layer only unless separately approved. A decision recorded here changes how something looks or is arranged, not what BUDCOM does, unless explicitly marked otherwise.
**Governance:** `POST-MVP-1-DEVELOPMENT-MODUS-OPERANDI.md`, `BUDCOM-MASTER-PRODUCT-EXECUTION-PLAN.md` (roadmap sequencing), `BUDCOM-PRODUCT-DECISION-LOG.md` (product-scope decisions)
**Companion file:** `docs/design/BUDCOM-SCREEN-INVENTORY.md`
...
(`BUDCOM-VISUAL-DESIGN-SYSTEM.md`). That file is intentionally not created
...
```
BUDCOM global shell → My Business
  - Profile
  - Catalogue
  - Business Library
```
...
(Business Profile / Catalogue) in `BUDCOM-MASTER-PRODUCT-EXECUTION-PLAN.md`
...
- **Presentation-layer only.** None of the above authorizes a new backend
  ...
  `BUDCOM-MASTER-PRODUCT-EXECUTION-PLAN.md`, not by this file.
...
- **External design archive stays outside the repository.** Approved visual
  ...
  `D:\BUDCOM-Design-Archive\` (`00_README_AND_DECISIONS`,
...
Not created yet, by design. `BUDCOM-VISUAL-DESIGN-SYSTEM.md` (tokens,
