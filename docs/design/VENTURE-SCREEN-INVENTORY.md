# VENTURE Screen Inventory

**Status:** Active — reflects `apps/venture_android` source as of the MVP-1.3 planning/recovery review
...
**Purpose:** Map every currently implemented Android screen to its relationship with the locked visual direction in `VENTURE-UI-DESIGN-DECISIONS.md`, so future implementation work knows what's unchanged, what's superseded, and what's net-new before touching source.
**Companion file:** `docs/design/VENTURE-UI-DESIGN-DECISIONS.md`
...
## Current screens (`apps/venture_android/app/src/main/java/com/jajusri/venture/feature/*/presentation/`)

| Screen | Source file | Relationship to new direction |
|---|---|---|
| Dashboard | `dashboard/presentation/DashboardScreen.kt` | **Partially redesigned** — the MVP-1 UI/UX polish pass (`docs/design/VENTURE-MVP-1-UI-UX-POLISH-STATUS.md` §2) added the A/c Data Home header, permanent Universal Search entry, compact sync/Tally-connected status row, and Vouchers/Ledgers primary entries per §3, reconciled against the approved Stitch master `VENTURE-AC-DATA-HOME-MASTER.png`, additively above the existing detail cards (none removed). The Insights surface and Connect/Vartalap bottom tabs remain **not yet built** — no backing data/destinations exist; still belongs to the dedicated Insights workstream. |
...
| Voucher Browser | `voucher/presentation/VoucherBrowserScreen.kt` | **Implemented** — unified chronological list (no per-type screens) with LEFT/CENTER/RIGHT row layout (UIP-001) and, as of `docs/design/VENTURE-MVP-1-UI-UX-POLISH-STATUS.md` §3, the compact horizontal All/Sales/Purchase/… type-filter strip per §5 — the strip was found missing during that pass despite UIP-001 being marked closed, and has now been built. |
...
| My Business (Profile / Catalogue / Business Library) | §7 | Owner-side Business Profile Ecosystem shell — maps to roadmap MVP-1.3 (Business Profile) and MVP-1.4 (Catalogue) in `VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md`. Not an MVP-1.0.x item. |
| Resources (visitor-facing Business Library view) | §7 | Permission-controlled view over the same owner-side data — must not fork a second data copy. Scope (whether it ships in MVP-1.3 at all) is an open product decision — see `docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md` §5.5. |
| Vartalap | Bottom tab, §3 | No dedicated roadmap milestone yet in `VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md` as of this consolidation — scope pending. |
