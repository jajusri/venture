# VENTURE MVP-1 UI/UX Polish — Status
...
`docs/design/VENTURE-UI-DESIGN-DECISIONS.md` for the separately-governed Home/Insights/Business-Profile
direction, which this pass does **not** implement beyond the one exception noted in §2.
**Companion files:** `docs/design/VENTURE-SCREEN-INVENTORY.md`,
`docs/planning/VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md`
...
`D:\VENTURE-Design-Archive\01_APPROVED_MASTERS\AC_DATA_HOME\VENTURE-AC-DATA-HOME-MASTER.png`, per the
storage plan at `D:\VENTURE-Design-Archive\00_README_AND_DECISIONS\VENTURE-UI-MASTER-STORAGE-PLAN.txt`.
That plan's "LOCKED DESIGN DECISIONS TO PRESERVE" section independently confirms the same layout
already recorded in `VENTURE-UI-DESIGN-DECISIONS.md` §3.
...
- The Insights card and its financial figures (Total Receivables, Cash in Hand, sparkline, Overdue
  Amount, Monthly Sales, Suggested Actions). VENTURE has no computation or data source for any of
  these today — `VENTURE-UI-DESIGN-DECISIONS.md` §4 itself calls this navigation-shape-only and
  explicitly the responsibility of a separate Insights workstream, and
  `VENTURE-ACCOUNTING-OPTIONAL-UX-PRINCIPLE.md` and §4 both say a fabricated/empty widget is worse
...
them off `VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md`'s roadmap into this app.
...
- **Voucher Details had no back-navigation affordance at all** (no `navigationIcon`, no `onBack`
  wired anywhere in `VentureNavHost`) — every sibling detail screen has one. Added, wired through
  `VoucherDetailsRoute`/`VentureNavHost` to `popBackStack()`. 1 new instrumented test.
- **The app's one shared loading component was dead code**, duplicated ad hoc at
  `MasterDataListComponents.MasterDataLoadingIndicator`, `VentureNavHost`'s root start-gate, and
...
- `ui/theme/Type.kt`'s `VentureTypography` only overrides 7 of Material 3's type roles
...
- App-wide spacing/padding magic numbers (6/10/13/14/18dp) scattered against a dominant
  ...
  per `VENTURE-UI-DESIGN-DECISIONS.md` §9's own stated sequencing (design system comes after more
...
- Desktop storage-gate dialog mixes button-label casing ("Rescan drives" / "Choose a different
  drive" sentence case vs. "Continue"/"Retry"/"Exit VENTURE" Title Case) — low-visibility, rarely
...
- **Desktop tray icon/menu.** The operating brief lists "tray/status behavior" among Desktop
  ...
  `VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md` §2 ("does this increase what MVP-1 does?"), adding
...
- **Desktop product naming: "Business OS Tally Connector"** (window title, `<h1>`, About page,
  ...
  **vs. "VENTURE"** (storage-gate dialogs). This is centralized in a named constant, not scattered
...
of this session (`docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md`,
...
Android (`apps/venture_android`, JAVA_HOME = Android Studio's bundled JBR 21, since the system
...
phone with live VENTURE state, and installing a new debug build/running automated UI interactions on
...
Desktop (`apps/venture_desktop`): `tsc -p tsconfig.renderer.json/main.json/preload.json --noEmit` —
...
1. Compare the implemented Android Home layout against `VENTURE-AC-DATA-HOME-MASTER.png` on a real
...
- `D:\VENTURE-Design-Archive\01_APPROVED_MASTERS\AC_DATA_HOME\VENTURE-AC-DATA-HOME-MASTER.png`
- `D:\VENTURE-Design-Archive\01_APPROVED_MASTERS\LEDGER_STATEMENT\VENTURE-LEDGER-STATEMENT-MASTER.png`
- `D:\VENTURE-Design-Archive\01_APPROVED_MASTERS\VOUCHER_DETAIL\VENTURE-VOUCHER-DETAIL-MASTER.png`
- Voucher density/filter guidance: the two images under
  `D:\VENTURE-Design-Archive\02_APPROVED_NEXT_SCREENS\VOUCHERS_UNIFIED\`.
...
- APK: `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk`, 14,261,943 bytes,
...
`0.1.1-continuity.22` (versionCode 23). That installed implementation is the primary living VENTURE
...
master exists in `D:\VENTURE-Design-Archive`, so the approved Android hierarchy, dark surface/color
...
- Product commit: `96a7a55` (`feat(desktop): apply approved VENTURE visual language`).
...
- Installer: `release/controlled-pilot/0.4.17/artifacts/VentureDesktop-0.4.17-x64-setup.exe`,
...
layer. Electron main was alive, preload exposed `window.ventureDesktop`, the private-storage overlay
...
Corrected installer:
`release/controlled-pilot/0.4.18/artifacts/VentureDesktop-0.4.18-x64-setup.exe` — 106,073,592 bytes,
