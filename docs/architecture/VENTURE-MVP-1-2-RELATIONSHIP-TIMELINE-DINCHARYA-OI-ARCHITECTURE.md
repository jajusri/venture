# VENTURE MVP-1.2 — Relationship Timeline, Issue History, Dincharya & OI Architecture
...
**Decisions locked:** 2026-08-18, `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` PDL-014–PDL-017 (§3 below)
**Governance:** `docs/governance/POST-MVP-1-DEVELOPMENT-MODUS-OPERANDI.md`
**Primary planning authority:** `docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md` §8 (locked scope title and objective)
**Supporting authority:** `docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md` §20/§23/§24 (the only repository document that defines what these four terms actually mean and where they sit in the UI)
**Frozen foundation this builds on:** MVP-1 through MVP-1.1-E, `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md`, HEAD `ad32f9a`

This document is MVP-1.2's equivalent of `VENTURE-MVP-1-1-CONNECT-UNIVERSAL-PARTY-ARCHITECTURE.md` — a
...
material — plus data VENTURE already has but doesn't yet surface (unconfirmed Tally exports, parties
...
- **No dedicated "Insights" specification exists anywhere in the repository.**
  `docs/design/VENTURE-UI-DESIGN-DECISIONS.md` §4 explicitly locks only the Home Insights
...
`docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` PDL-014 through PDL-017. **All four locked
...
| Field provenance (Tally-confirmed vs. VENTURE-pending) | ✅ | | | `PartyFieldProvenance` (1.1-A/D) | — | Reused as Dincharya's "pending Tally confirmation" source, zero new schema |
...
**The single most important user outcome:** a shop owner who has been noting things down in VENTURE
...
written, and without VENTURE inventing anything that isn't grounded in data the owner actually
...
- **Generic CRM pipeline, lead scoring, relationship-health percentage, task manager** — explicitly
  forbidden by `docs/planning/VENTURE-NOT-NOW.md` and the referral spec §28.
- **Any generative-AI-backed suggestion or scoring** — explicitly forbidden by `VENTURE-NOT-NOW.md`
  ("Advanced generative AI dependency... OI/deterministic helpers first") and the OI positioning
  itself.
- **Vartalap integration** — no milestone exists yet (confirmed via `VENTURE-SCREEN-INVENTORY.md`).
...
Authority: `docs/design/VENTURE-UI-DESIGN-DECISIONS.md` (locked visual character — cutting-edge
...
- OI framing line near the top or in the empty state: language consistent with *"We are not AI.
  This is OI — programmed to help you"* — plain, honest, never implying autonomous judgment VENTURE
...
Connect surface of any kind (`Connect`/`Party` string matches in `apps/venture_desktop/src/renderer`
...
issues, due dates, export/provenance state) is VENTURE-local. The one Tally-adjacent Dincharya item
...
- **Company isolation is the dominant security concern for MVP-1.2** (§13 Risk #1) — Dincharya's
  ...
  isolation tests).
- No new sensitive-data surface: issue titles and note bodies are user-authored VENTURE-only text,
...
> **VENTURE MVP-1.2-A — Structured Party Activity Foundation**
>
> Repository: `C:\Projects\Venture`. MVP-1.1 is COMPLETE/FROZEN at HEAD `ad32f9a` — do not reopen it
> unless you discover a genuine blocking defect. Read
> `docs/architecture/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-OI-ARCHITECTURE.md` in full
> before starting — it is this milestone's locked scope/architecture baseline, reconciled against
> actual repository evidence. Also read `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md` and
> `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` for current state.
>
...
- `docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md` §8 (locked MVP-1.2 title/objective — primary authority)
- `docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md` §14–§28 (only source that defines these terms and their UI placement)
- `docs/architecture/VENTURE-MVP-1-1-CONNECT-UNIVERSAL-PARTY-ARCHITECTURE.md` §36 ("Relationship to later roadmap")
- `docs/design/VENTURE-UI-DESIGN-DECISIONS.md` (locked visual character, Home Insights nav-shape-only boundary)
- `docs/design/VENTURE-SCREEN-INVENTORY.md` (confirms no screen exists yet for any MVP-1.2 surface, confirms Vartalap has no milestone)
- `docs/planning/VENTURE-NOT-NOW.md` (explicit scope exclusions — CRM, AI dependency, dialer/call-log)
- `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` (PDL-012 complexity discipline, PDL-013 simple-experience discipline)
- `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md` Parts A–E, `docs/status/VENTURE-DEVELOPMENT-LEDGER.md`, `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md` (frozen baseline)
- Direct repository inspection: `PartyEntities.kt`, `PartyModels.kt`, `DatabaseModule.kt`, `AndroidManifest.xml`, `apps/venture_desktop/src/renderer/**` (confirmed no Connect/Party surface), `PartyRepository.kt`
