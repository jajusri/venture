...
BUDCOM today has **no automatic Tally synchronization anywhere** — Desktop, Android, and the
...
external-API question, not a BUDCOM-code question). What **is** achievable today, safely, using only
...
### 2.1 Desktop (`apps/budcom_desktop`)
...
### 2.2 Connector (`connector/budcom_connector`)
...
### 2.3 Android (`apps/budcom_android`)
...
BUDCOM cannot currently determine, without a full extraction, whether Tally has changed, what
...
**Can BUDCOM detect "Tally changed since last sync" without downloading/reconstructing the entire
...
codebase — this is external ERP-integration knowledge, not something found in BUDCOM's source) to
expose an `ALTERID`/`ALTERMASTERID`/`ALTERVOUCHERID` concept at both the record and company level,
which third-party Tally integrations sometimes use for cheap change polling. BUDCOM already fetches
...
assume it will work against BUDCOM's actually-supported Tally versions/editions.
...
- **"Finished working in Tally? Sync now" contextual suggestion**: there is no reliable way today to
  ...
  request/response, not a persistent session BUDCOM observes end-to-end. Per the governing task's own
  ...
  honestly as "the connection dropped," not "you finished your session" (BUDCOM cannot know the
...
adaptive scheduler adds exactly one new fact the user might reasonably want ("is BUDCOM checking often
