# VENTURE MVP-1.3 — Business Profile — Architecture
...
Owner and are now recorded as `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` PDL-019. §5's
...
`docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` for implementation evidence.
...
(`docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md` §9). Its **ownership/visual shape** is
locked (`docs/design/VENTURE-UI-DESIGN-DECISIONS.md` §7): one business identity, one catalogue, one
...
- Last 8 commits confirmed to be exactly the MVP-1.2-D/E sequence (`f049fb1` through `7732844`),
  matching `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` phases 31–32 without discrepancy.

### 2.2 MVP-1.2 freeze confirmation

- `apps/venture_android/app/build.gradle.kts`: `versionCode = 28`, `versionName = "0.1.1-continuity.27"`
...
- Connector: zero endpoint plausibly related to business-profile/catalogue data. Consistent with the
  Connector's own locked boundary (100% read-only Tally proxy) — a Business Profile is VENTURE-native
...
`docs/design/VENTURE-SCREEN-INVENTORY.md`'s "Current screens" table predates MVP-1.1-B — it lists
...
`docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md` §9, verbatim: *"Detailed scope comes from
...
A full search of `docs/` and the external design archive (`D:\VENTURE-Design-Archive\` — confirmed to
...
> Website, Logo, Short Business Description). §5.3→no `PartyFieldProvenance`, 100% VENTURE-owned data.
...
Every piece of VENTURE data built so far (Party, notes, issues, Dincharya, vouchers, ledgers) is
...
the nav shape is a single global `My Business` shell, not `Company X → My Business`. A real VENTURE
...
Tally-sourceable (and thus needs the existing `FieldProvenance` pattern) or VENTURE-only.

### 5.3 Editable-vs-Tally-derived semantics, and whether the existing `PartyFieldProvenance` model applies at all

Party's field-provenance model (`ConfirmedFromTally`/`VentureOnlyPending`/`ExportReady`/`Exported`/
...
VENTURE-authored, provenance-free data)? If entirely VENTURE-only, the whole provenance/export
...
VENTURE has no image-upload precedent anywhere today (only image *display*/*share* via the existing
...
(`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §9, survives reinstall, has its own fail-closed
...
- Offline-capable by construction (matches every existing VENTURE screen — no document suggests an
  exception).
...
- Generative AI of any kind (`docs/planning/VENTURE-NOT-NOW.md` "Advanced generative AI dependency";
  PDL-016).
...
- Public marketplace / social feed (`VENTURE-NOT-NOW.md`, and Master Plan §9's own explicit warning
...
| `businessName` | TEXT | VENTURE-only unless a real Tally company-name field exists to reconcile against (§5.3) |
...
`ventureValue`, `lastConfirmedAt`, `lastExportedAt`, `updatedAt`) — same reasoning as MVP-1.1-A's own
"normalized per-field rather than one provenance column per field" decision. **Do not build this table
if §5.3 concludes Business Profile is VENTURE-only data with no Tally round-trip** — it would be
...
existing VENTURE screen, no document suggests otherwise, and no network dependency should be
...
prompt directly, the same way `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md`
...
`docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md`, `docs/design/VENTURE-UI-DESIGN-DECISIONS.md`,
`docs/design/VENTURE-SCREEN-INVENTORY.md` (corrected, §2.4), `docs/planning/VENTURE-NOT-NOW.md`,
`docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` (PDL-001 through PDL-018 read in full),
`docs/governance/POST-MVP-1-DEVELOPMENT-MODUS-OPERANDI.md`, `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md`,
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md`, `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md`,
`docs/architecture/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-OI-ARCHITECTURE.md` (as the direct
structural precedent for this document), a full repository source search (Android/Desktop/Connector),
and a direct inspection of the external design archive at `D:\VENTURE-Design-Archive\` (confirmed to
