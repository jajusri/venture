# BUDCOM Transaction Mode (Vartalap-derived) — Architecture Hardening Pass
...
`docs/architecture/BUDCOM-VARTALAP-TRANSACTION-MODE-BRAINSTORM-OUTCOME.md`'s locked Q1–18 design.
...
(`BUDCOM-VARTALAP-TRANSACTION-MODE-BRAINSTORM-OUTCOME.md`) does not exist anywhere in this
repository, its git history, or `Budcom_archives`** (confirmed by direct search this session). It was
...
the same way `BUDCOM-MVP-1_4-CATALOGUE-BRAINSTORM-OUTCOME.md` resolves today — this session did not do
...
BUDCOM installations — the buyer's and the seller's, each its own `companyId`, each its own local Room
...
precedent explicitly rules one out.** Direct quote, `docs/architecture/BUDCOM-MVP-1-4-CATALOGUE-ARCHITECTURE.md`
...
session by an exhaustive grep of `apps/budcom_android/app/src`: no `NotificationChannel`, no
`NotificationCompat`, no `NotificationManager`, no FCM, no `NotificationType` enum, and — despite
`WorkManager` being wired into `BudcomApplication.kt` for Hilt DI reasons — **zero `Worker`/
...
| `LedgerEntity` | `cached_ledgers`, PK `(companyId, id)` — no write path exists anywhere in this app; BUDCOM only ever *exports* enrichment XML for a human to import into Tally (`PartyExportEventEntity` / `TallyLedgerXmlGenerator`). Confirmed: **no code path creates a Tally Ledger from BUDCOM.** |
...
- **No Tally write occurs.** Confirmed this session: BUDCOM has no code path that creates a Tally
...
**Implementation-confirmed addendum (Phase 64, `docs/status/BUDCOM-DEVELOPMENT-LEDGER.md` §55):**
...
**No money is ever processed by BUDCOM** — there is no payment-gateway integration, no card/UPI
...
company. A buyer using their own BUDCOM installation would need their own local copy of (at minimum)
...
7. **Buyer-side data ownership when the buyer has no BUDCOM installation at all.** Every table in this
   ...
   BUDCOM installation (plausible for many small buyers, especially via the WhatsApp-shared path),
   ...
   "buyer: simple upcoming-due tracking" view is implicitly scoped to buyers who **do** run BUDCOM —
...
`BUDCOM-VARTALAP-TRANSACTION-MODE-BRAINSTORM-OUTCOME.md` (supplied directly this session — not
present in the repository; see the status note at the top of this document),
`docs/architecture/BUDCOM-MVP-1_4-CATALOGUE-BRAINSTORM-OUTCOME.md`,
`docs/architecture/BUDCOM-MVP-1-4-CATALOGUE-ARCHITECTURE.md` (in full, both halves),
`docs/governance/BUDCOM-PRODUCT-DECISION-LOG.md` PDL-020 (`:280-362`), and direct repository
...
`app/BudcomApplication.kt`, `connector/budcom_connector/src/services/scheduler/adaptive-scheduler.service.ts`,
...
inspected directly via `git log --oneline -- apps/budcom_android/.../feature/catalogue/` to establish
