# BUDCOM MVP-1.1 — Connect + Universal Party Identity Architecture
...
**Repository target:** `D:\Projects\Budcom`
...
- enrich missing contact details in BUDCOM;
- distinguish Tally-confirmed fields from BUDCOM-only pending fields;
- generate reviewed Tally-compatible XML for eligible fields;
- import that XML into Tally manually;
- re-sync;
- see fields become confirmed only after BUDCOM reads the same value back from Tally;
- keep BUDCOM-only notes, tags and extra contact persons without forcing them into Tally.
...
BUDCOM does **not** directly write to Tally.

Approved write-back boundary:

**BUDCOM edit → review → generate Tally-compatible XML → user imports into Tally → re-sync → confirm**
...
```text
Party
 ├─ partyId                 stable BUDCOM UUID/identifier
 ├─ companyScope            BUDCOM/Tally company scope
 ├─ displayName
 ├─ partyClassification
 │   ├─ Customer
 │   ├─ Prospect
...
A BUDCOM Party is the durable business/contact identity used across BUDCOM.
...
- ledger names change;
- extra BUDCOM-only contact persons exist;
- BUDCOM-only tags/notes exist;
...
For the initial MVP-1.1 rollout, create/derive BUDCOM Parties from the existing Tally-backed ledger/contact universe according to explicit eligibility rules.
...
- **BUDCOM_ONLY / PENDING** — value exists in BUDCOM but has not been confirmed from Tally.
...
- **CONFLICT** — Tally later returned a different non-empty value than the BUDCOM pending/exported value.
- **EMPTY / UNKNOWN** — no value known.

UI visual rule remains:

- **Green = confirmed from latest Tally sync**
- **Black = BUDCOM-only/pending**
...
2. the returned value matches the BUDCOM value according to field-specific canonical comparison rules.
...
If BUDCOM pending value and latest Tally value differ:
...
Do not create conflict complexity for fields that are BUDCOM-only and never intended for Tally.

---

## 7. Tally-backed fields vs BUDCOM-only fields
...
Implementation must inspect the actual Tally schema currently supported by BUDCOM and not invent unsupported fields.

### 7.2 BUDCOM-only fields
...
- BUDCOM-specific preferences.

BUDCOM-only fields must not be pushed into Tally XML.
...
If Tally Alias is **exactly 10 digits**, BUDCOM may seed/copy it into the BUDCOM phone field non-destructively.
...
Extra people may remain BUDCOM-only.
...
Support BUDCOM-owned hierarchical and classification tags.
...
- Call
- WhatsApp
- BUDCOM Message / Vartalap placeholder only if existing product design reserves it safely
- View Ledger
- View Vouchers
...
Use existing approved BUDCOM WhatsApp/share conventions.
...
Party field confirmed
   ↓
User edits/adds eligible field in BUDCOM
   ↓
Field becomes pending
   ↓
User reviews changes
   ↓
BUDCOM generates Tally-compatible XML
   ↓
...
- proposed value;
- conflicts;
- fields excluded because they are BUDCOM-only.

### 18.3 No automatic import

BUDCOM prepares the XML.
...
- Tally ledger renamed after BUDCOM edit;
...
Party identity is BUDCOM-stable, but Tally source links and accounting context remain company-scoped.
...
- read notes/tags;
- edit BUDCOM-only/pending fields;
...
BUDCOM Party entity with field-level source/provenance.

### Tally-confirmed contact fields
Latest synchronized Tally data.

### BUDCOM-only fields
BUDCOM local source of truth.

### Notes/tags/contact persons
BUDCOM source of truth unless explicitly mapped to a Tally-compatible primary field.
...
**Design irreversible decisions as though BUDCOM may one day serve a billion users. Build reversible implementations only for the scale evidence justifies today.**
...
Connect starts from **Tally/BUDCOM ledger contacts**, not the Android device phonebook.

Do not request Contacts permission merely to implement MVP-1.1.

Do not import phonebook contacts.

Prospects must be BUDCOM-managed records created through approved BUDCOM flows, not bulk-ingested device contacts.
...
- BUDCOM-only data never accidentally exports to Tally.
...
Use the approved BUDCOM Android visual language established in MVP-1.
...
- Party Detail supports Tally-backed and BUDCOM-only fields;
...
- `docs/status/BUDCOM-CURRENT-DEVELOPMENT-STATUS.md`
- `docs/status/BUDCOM-DEVELOPMENT-LEDGER.md`
- `docs/planning/BUDCOM-MASTER-PRODUCT-EXECUTION-PLAN.md`
- `docs/governance/POST-MVP-1-DEVELOPMENT-MODUS-OPERANDI.md`
- `docs/governance/BUDCOM-CLAUDE-PROMPT-PROCESSING-RULES.md`
- `docs/governance/BUDCOM-PRODUCT-DECISION-LOG.md`
- `docs/planning/BUDCOM-NOT-NOW.md`
