# VENTURE Ground-Truth Snapshot — 2026-09-13

**Produced by:** Claude, working directly against the `venture` repo clone (this project's
Claude-only fork of Venture) — by reading git history and running live test suites, not by
trusting prior status documents. The two prior canonical documents
(`VENTURE-CURRENT-DEVELOPMENT-STATUS.md`, `VENTURE-DEVELOPMENT-LEDGER.md`) both stop at **Phase 71
(2026-08-25)** and do not mention any of the work described below — they are stale as of this
snapshot. `VENTURE-GROUND-TRUTH-PROJECT-STATE-AUDIT.md` (2026-08-23) is the prior snapshot this
one supersedes.

**Repo/branch:** `main`, in sync with `origin/main` (0 ahead, 0 behind). This branch is the old
Venture repo's `integration/final-mobile-desktop` branch, renamed — it is materially ahead of what
...
"**separately deployable VENTURE central services... not part of the LAN/Tally Connector**."
...
business's app. This is the one piece of VENTURE's architecture that is genuinely multi-tenant
...
| Desktop | **710 passed, 31 failed** (69 files) | Same pattern — the failures shown are Windows path/Electron-mock assumptions (`C:\Program Files\Venture Desktop\...`, `app.setPath` override) that don't hold on Linux. **Needs re-confirmation on Windows.** |
