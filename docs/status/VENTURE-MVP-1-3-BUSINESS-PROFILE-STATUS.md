# VENTURE MVP-1.3 — Business Profile — Status
...
`docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md` (planning/recovery review this
milestone builds on), `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` PDL-019 (the five locked
...
`docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` **PDL-019**, mirroring the format of PDL-014
through PDL-018:
...
3. Business Profile is 100% VENTURE-owned data for MVP-1.3 — no `PartyFieldProvenance`, no Tally
...
zero files under `apps/venture_desktop` or the Connector changed; zero `AndroidManifest.xml`
...
`app/schemas/com.jajusri.venture.core.database.AppDatabase/10.json` after a real build — every
...
- **Accidental Tally write / network call / Desktop/Connector/catalogue/public-exposure surface:**
  ...
  `AndroidManifest.xml` change; zero file under `apps/venture_desktop` touched.
...
- **No accidental Tally/network/Desktop/Connector/manifest/public-exposure surface anywhere in A+B
  ...
  diff, zero file under `apps/venture_desktop` touched).
...
`apps/venture_android/app/build.gradle.kts`: `versionCode` `28` → **`29`**, `versionName`
...
**Debug:** `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` — **14,562,380 bytes** —
...
`docs/planning/VENTURE-MVP-1-PUBLIC-RELEASE-GATE-MATRIX.md`).
...
com.jajusri.venture`) confirmed the app was indeed absent post-test-run — `connectedDebugAndroidTest`'s
...
**Install performed:** `adb install -r apps/venture_android/app/build/outputs/apk/debug/app-debug.apk`
...
**Smoke test performed directly on device:** launch succeeded (`monkey -p com.jajusri.venture.debug
...
post-push verification are recorded in `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md` §2/§2a.
