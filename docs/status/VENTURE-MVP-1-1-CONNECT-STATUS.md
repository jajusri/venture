# VENTURE MVP-1.1 — Connect Status
...
- `docs/architecture/VENTURE-MVP-1-1-CONNECT-UNIVERSAL-PARTY-ARCHITECTURE.md` (frozen architecture,
  adopted as-is — no edits required),
- `docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md` (locked product
...
- `docs/product/ux-ui/VENTURE-ACCOUNTING-OPTIONAL-UX-PRINCIPLE.md` (2026-08-14, LOCKED) and
  `docs/product/ux-ui/VENTURE-CONNECT-IMPLEMENTATION-CHECKLIST-v2.md` — UI-facing, but their
  ...
  link), so a future VENTURE-native Prospect can exist with zero Tally linkage. No code conflict —
  the schema already satisfied this before the principle was read; documented here for traceability.
- `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md` §10 listed the architecture file, the spec
...
identity (`connector/venture_connector/src/extraction/core/ledger-identity.ts`,
...
States: `confirmed_from_tally`, `venture_only_pending`, `export_ready`, `exported`, `conflict`,
`empty_unknown`. `PartyRepository.updateVentureOnlyField` always lands as `VentureOnlyPending` (never
confirmed merely because a user typed it); `confirmFieldFromTally` is the only path to
`ConfirmedFromTally`, and a mismatching pending VENTURE value becomes `Conflict` rather than being
...
provenance (`confirmed_from_tally` vs `venture_only`, not per-field, since only the Tally-compatible
...
contacts, primary-first ordering, VENTURE-only provenance, company+party scoping).
...
`Prospect` — a Prospect is, by the locked spec's own definition (§4.1), a VENTURE-created party not
...
`getFieldProvenance`, `updateVentureOnlyField`, `confirmFieldFromTally`,
...
`apps/venture_android/app/build.gradle.kts`. Bumped because production code (Room schema, DAOs,
...
| Build | Path | SHA-256 |
|---|---|---|
| Debug APK | `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` | `3bdbdc3e74ad246514488f928a066b3407ae592b912394b7d81e0a0d2ba40f48` |
| Release APK (unsigned — no release keystore exists, per current-status §3) | `apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` | `171169356b8ed51efce1c1bc717750e766f8aaedd72497a4003b59369d113583` |

## 19. ADB / install status

A device was connected during this session (`I2407i`, model `I2407`) — used to run the 23
instrumented tests in §13/§15 (installed to the debug-suffixed `com.jajusri.venture.debug` package,
which cannot collide with any production install). `pm list packages`/`dumpsys package` confirmed
**no VENTURE package of any kind is installed on this device** — it is not the previously-approved
...
registered in `VentureNavHost`. Reached from a new third `HomePrimaryEntryRow` on the Dashboard
...
accounting source link — never fabricated for a VENTURE-only/Prospect Party). Uses
`MaterialTheme.colorScheme`/`typography` tokens throughout (no hardcoded colors), inheriting
`VentureTheme`'s existing light/dark support for free. Mini-hardening: name and balance are
...
is independent of VENTURE's own network state).
...
|---|---|---|---|
| Debug APK | `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` | `50f80bede5d990c5327736d23f02844570e2d9696d3f0010f679caaed9b793c4` | 14,119,040 bytes |
| Release APK (unsigned — no release keystore exists) | `apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` | `867596a8d7b0ac48bd4053ab4bc1bee912e4bb8946e4d9d1f6038428f59c45a2` | 2,387,924 bytes |

## B23. ADB device / install result

Connected device: `I2407i` (model `I2407`, serial `10BF44124K000E3`). `pm list packages` showed
**no pre-existing `com.jajusri.venture` (production) package** — the only VENTURE-named packages
present (`com.jajusri.venture.debug`, `com.jajusri.venture.debug.test`) were installed by this
...
instruction's explicit rule ("if no connected device has VENTURE installed: do not install; report
...
contact persons, tags, and VENTURE-only notes (including voucher-linked notes) — the first slice
where a user can *edit* Party data in VENTURE, not just browse it.
...
spec's own definition), seeds a `PartyFieldProvenance` row at `VentureOnlyPending` for every
...
- **B/C. Contact fields with provenance** — each Tally-compatible field (`Phone`/`Email`/
  ...
  `FieldProvenanceState.toUiLabel()`: "Confirmed from Tally" / "Pending in VENTURE" / "Ready to
  ...
  Tapping a field opens an edit dialog; saving always calls `updateVentureOnlyField` (1.1-A), which
  always lands as `VentureOnlyPending` — a user typing a value can never mark it confirmed by
...
"connect/prospect/new"`, both registered in `VentureNavHost`. `ConnectScreen`'s row `Card` gained
...
already exist via Tally seeding and are never VENTURE-created.

## C8. Field-editing safety (no accidental Tally writes)

`updateVentureOnlyField` is the **only** write path a Party Detail edit can reach — it never touches
...
- **Provenance:** an edited field always lands `VentureOnlyPending`, never silently confirmed (C5/
...
- **Light/dark:** every new composable uses `MaterialTheme.colorScheme`/`typography` tokens only —
  no hardcoded color was introduced; inherits `VentureTheme`'s existing light/dark support.
...
| Build | Path | SHA-256 | Size |
|---|---|---|---|
| Debug APK | `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` | `6a4a40616662e2a2240a105ec65f5c17d7542e6cc06f0a1ebc6a2a648650b470` | 14,690,298 bytes |
| Release APK (unsigned — no release keystore exists) | `apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` | `4e42598b39f970be62e7ebcacc5974b7f2b07669116de0d83ce8b70158b8fe53` | 2,437,076 bytes |

## C16. ADB device / install result

Connected device: `I2407i` (model `I2407`, serial `10BF44124K000E3`) — the same device used in
Parts A/B. `pm list packages` again shows **no VENTURE package of any kind** on this device (the
...
**Status:** Technically complete, ready for review. VENTURE can now review pending/conflicted
...
VENTURE never writes to Tally directly at any point.
...
doesn't exist and a field VENTURE could never honestly re-confirm from a genuine Tally read-back —
excluded rather than faked. Every VENTURE-only field (tags, notes, extra contact persons,
...
**Known, explicitly disclosed Tally XML limitation, not a VENTURE shortcut:** Tally's XML
...
**Edit-after-export staleness handled for free, not as a special case:** `updateVentureOnlyField`
(1.1-A, unchanged) always resets a field's state to `VentureOnlyPending` on any new edit, regardless
...
- **Edit-after-export staleness:** verified to fall out correctly from 1.1-A's existing
  `updateVentureOnlyField` behavior with zero new bookkeeping (§D6).
...
- **Conflict UX:** the change-review screen shows both the last-known Tally value and the current
  VENTURE value side by side per field when both exist — no silent choice, no merge engine.
...
| Build | Path | SHA-256 | Size |
|---|---|---|---|
| Debug APK | `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` | `8619c5652555a56616d78ef8ace58b8be87e74f89568dfba5b9ac5689da915c6` | 14,332,080 bytes |
| Release APK (unsigned — no release keystore exists) | `apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` | `c34a05902e150fe5bb4ce22fdd086cc279a9cf1d0bd72ecfe902d90d02bede86` | 2,469,892 bytes |
...
shows **no VENTURE package of any kind** on this device at every check this session. Per the
governing instruction's explicit rule, **no distinct "final candidate" install was performed** —
...
2. Edit a Tally-compatible field (e.g. Email) — confirm it shows "Pending in VENTURE."
...
- No physical Tally round-trip was performed this session (per explicit instruction — VENTURE must
...
Read `VENTURE-CURRENT-DEVELOPMENT-STATUS.md`, `VENTURE-DEVELOPMENT-LEDGER.md`, this file (Parts
...
**no VENTURE package of any kind installed** — every value matched the expected A–D baseline
...
- **Navigation graph** (`navigation/Routes.kt`, `navigation/VentureNavHost.kt`): every route
...
- **Provenance gating**: grepped the full `feature/connect` presentation layer for
  ...
  Part D §D8 claimed. A VENTURE edit can still never mark itself Tally-confirmed.
...
| Build | Path | SHA-256 | Size |
|---|---|---|---|
| Debug APK | `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` | `8a3413e7b4e4a22254fab7d2cbf05a698e22052ce6ef631e1e6dc6373282c4c3` | 14,348,468 bytes |
| Release APK (unsigned — no release keystore exists) | `apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` | `aa0c037cb005757fca66173ff3cb30fdaa8c981acd53488ae4b00105cd219e91` | 2,469,896 bytes |

## E13. ADB device / install result

Device `I2407i` (model `I2407`, serial `10BF44124K000E3`) connected throughout this session (used
for §E6's instrumented run). `pm list packages | grep venture` returned **no VENTURE package of any
...
instruction's explicit rule (§19/§22: the target must already contain VENTURE data or be otherwise
clearly identified as the owner's existing VENTURE phone; do not install onto an unrelated/unproven
