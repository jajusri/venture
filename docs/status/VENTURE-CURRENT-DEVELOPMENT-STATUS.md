# VENTURE Current Development Status

**Status:** Canonical concise current-state checkpoint. The historical/audit record is
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md`; detailed milestone evidence lives in specialist status
documents (e.g. `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md`). This file stays short and links
...
freeze) are all complete. Product decisions locked as `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md`
PDL-014–PDL-018. Full detail: `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md`
Parts A–E; architecture: `docs/architecture/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-OI-ARCHITECTURE.md`.
...
`docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` **PDL-019**. Part A shipped the `business_profile`
...
`docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Parts A–C;
architecture: `docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md` (its §3/§5 open
...
`docs/architecture/VENTURE-MVP-1_4-CATALOGUE-BRAINSTORM-OUTCOME.md`'s Final Scope Lock is
...
not implied by "COMPLETE" here. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §54 (Phase
...
`docs/architecture/VENTURE-TRANSACTION-MODE-ARCHITECTURE.md` (RECOMMENDED, not LOCKED, same status as
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §55 (Phase 64).
...
built the Dev APK: `apps/venture_android/app/build/outputs/apk/dev/debug/app-dev-debug.apk`. 108
...
`apps/venture_android/app/build/outputs/apk/dev/debug/app-dev-debug.apk`. Seller Inbox UI
deliberately not attempted. **Phase 71** — physically installed on the user's own device over the
existing paired `com.jajusri.venture.debug` install (data preserved) and manually tested against real
...
fixes. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §56–62 (Phases 65–71).
...
instrumented verification. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §24.
...
independent of this session. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §26.
...
existing-Ledger linking capability in `docs/planning/VENTURE-NOT-NOW.md` per the exact specified
concept, not implemented. Produced the MVP-1.4 planning/recovery review,
`docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md`, separating LOCKED scope (ownership
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §28.
...
relationship — VENTURE-owned presentation entity referencing Tally, no write-back; lifecycle Stock
...
`docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` §5.3 to mark all nine `RESOLVED —
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §29; `docs/technical-debt/registry.md` TD-035;
`docs/governance/VENTURE-PRODUCT-DECISION-LOG.md` PDL-020.
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §30.
...
(`VENTURE_INSTALLED_PROBE_MODE`/`VENTURE_USER_DATA_DIR`) — four independent launches of the dev build
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §30-H.
...
Connector-protocol/Catalogue scope touched. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md`
§31.
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §32.
...
assumption. Headline finding: **no automatic sync exists anywhere in VENTURE today** — the Connector's
...
`docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md`: a staged time-based backoff state
...
MVP-1.4 not started. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §33.
...
(`192.168.29.x`) instead of the mobile-hotspot SSID that blocked [[project_venture_td035_physical_validation]]'s
...
Desktop, or Connector production code touched. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md`
...
`docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md`: a pure state machine (5-minute
...
second sync engine anywhere. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §35;
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §36; `docs/technical-debt/registry.md` TD-039.
...
immediately against VENTURE's schema: already safe (`(company_id, guid)` composite unique indexes),
no existing defect, recorded as binding evidence for any future work. No production code changed.
Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §37;
`docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md` §3.1, §17 item 10 now CLOSED.
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §38-39; `docs/technical-debt/registry.md` TD-040.
...
linking. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §40.
...
uninstalled `com.jajusri.venture.debug` as Gradle's own managed lifecycle (flagged as a known risk in
...
detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §41.
...
`venture_desktop`): build succeeded, but the real Electron GUI could not launch because
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §42.
...
to `%APPDATA%\@venture\desktop` (LAN-bound, `192.168.29.34:8080`, private-removable vault on drive
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §43; `docs/technical-debt/registry.md` TD-041.
...
this phase. Full detail: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §44.
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §45.
...
- This is an explicitly **read-only reconnaissance session** — no production code, migration, test,
  ...
  new `docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md`, a correction to
  `docs/design/VENTURE-SCREEN-INVENTORY.md` (five stale-missing screens added), and this file plus
...
machine) — see `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §30-H for full detail. No defect found, no
...
Full detail (B–C): `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Parts B–C.
Prior milestone (MVP-1.1-A through E): `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md` Parts A–E.
...
`docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` "Checkpoint" section.
...
`docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Part D; ledger: phase 31;
...
`docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Part E; ledger: phase 32.
...
`VENTURE-SCREEN-INVENTORY.md`'s screen table had been stale since before MVP-1.1-B (missing five
shipped screens), corrected directly. Full detail:
`docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md`; ledger: phase 33.
...
`docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Part A; product decisions: PDL-019.
...
(deferred to Part C). Full detail: `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Part B.
...
`docs/planning/VENTURE-MVP-1-PUBLIC-RELEASE-GATE-MATRIX.md`. Not touched by MVP-1.1 or MVP-1.2 work.
...
com.jajusri.venture` that the app was indeed absent after the final `connectedDebugAndroidTest` run
(not assumed), then `adb install -r apps/venture_android/app/build/outputs/apk/debug/app-debug.apk`
→ `Performing Streamed Install / Success`. Confirmed via `dumpsys package
com.jajusri.venture.debug`: `versionCode=29`, `versionName=0.1.1-continuity.28`,
...
**Update (Phase 37, `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §26):** the "no real paired Tally
...
see Phase 36/37 in the Development Ledger for their own SHA-256s):** `apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` — 14,562,380 bytes —
SHA-256 `601b248fb07da27da2d805ab4ed8c06681b02e3c01eaeab22fc3a2994896902f` — `com.jajusri.venture.debug`,
...
`apps/venture_android/app/build/outputs/apk/release/app-release-unsigned.apk` — 2,536,316 bytes —
SHA-256 `dd68728630fb29f54b38ffb60aa8ca8264339810c9b2a7b9921c54b56be94a67` — `com.jajusri.venture`
...
com.jajusri.venture.debug`: `lastUpdateTime` advanced to `2026-08-19 07:48:12` while
...
monkey -p com.jajusri.venture.debug -c android.intent.category.LAUNCHER 1` (the plain `am start
-n .../MainActivity` component name used by earlier sessions no longer resolves — the real launcher
activity is `com.jajusri.venture.app.MainActivity`, not `com.jajusri.venture.MainActivity`; recorded
here for future sessions) — confirmed `mFocusedApp` became
`com.jajusri.venture.debug/com.jajusri.venture.app.MainActivity`, no `FATAL EXCEPTION`/`AndroidRuntime`
...
`apps/venture_android/app/build/outputs/apk/debug/app-debug.apk` — 15,185,874 bytes — SHA-256
...
evidence in `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Parts A–E).
...
`docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Parts A–C). The five product decisions this
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §24) and Phase 37 (Ledger Local-First + Connect
Real-Device Validation — §1 above, `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §26) are both
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §28) is complete.** Phase 36/37's commits are pushed
...
`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §29) is complete. This supersedes both items Phase 38 left
...
open and out of scope. All nine of `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` §5.3's
...
staff identity anywhere in VENTURE — `isOwner` hardcoded `true`), no branch-selector or
category-picker UI. Full itemized record: `docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §47. **Not
...
2. **Exercise a real Tally XML import by hand**, per the human acceptance checklist in
   `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md` Part E §E14 — **partially superseded by Phase 37**
   (`docs/status/VENTURE-DEVELOPMENT-LEDGER.md` §26): device pairing is confirmed genuinely done
