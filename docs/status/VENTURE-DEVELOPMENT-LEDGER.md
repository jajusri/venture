...
**Current-state companion:** `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md`

## 1. Purpose and evidence rules

This ledger explains how VENTURE reached its present state. It is the durable development history;
...
| Public release | **BLOCKED — signing.** No Windows code-signing or Android release-signing credentials exist. TD-033/TD-034 resolved this session; Android public `applicationId` flagged, not decided. See `docs/planning/VENTURE-MVP-1-PUBLIC-RELEASE-GATE-MATRIX.md`. |
...
`D:\VENTURE-Design-Archive\01_APPROVED_MASTERS\AC_DATA_HOME\VENTURE-AC-DATA-HOME-MASTER.png`, added
...
(TD-027); that is an upstream timing observation, not a VENTURE defect.
...
The physical session stopped VENTURE, removed USB, observed the unavailable screen, reinserted it,
...
| 027 | Tally export timing | Export may lag Tally UI | **OPEN OBSERVATION P2**, not VENTURE defect; reviewed 2026-08-17, no UI change made — a deliberate prior no-workaround decision stands, no new evidence to reverse it |
...
- VENTURE is local-first. Connector SQLite and Android Room hold durable, company-isolated state;
...
- **MVP-1.1-A (Universal Party Foundation).** Party identity/source-link/provenance/contact-person/
  ...
  `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md` Part A.
...
- **MVP-1.2: Relationship Timeline, Issue History, Dincharya and deterministic OI — COMPLETE /
  ...
  `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Parts A–E; concise summary
  and exact regression numbers per part: `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md` §3. (This
...
- **MVP-1.3: Business Profile — COMPLETE / FROZEN (2026-08-18).** Parts A, B, and C (integrated
  ...
  Full evidence: `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Parts A–C; see phase 35
  below.
...
credentials (see `docs/planning/VENTURE-MVP-1-PUBLIC-RELEASE-GATE-MATRIX.md` §6 for the exact setup
...
see phase 24 and `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md` Part E. This signing/applicationId
...
`docs/architecture/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-OI-ARCHITECTURE.md`. Before
...
- **MVP-1.2-D (Dincharya):** three locked deterministic item types (PDL-018) — Follow-ups/
  ...
  `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Part D.
- **MVP-1.2-E (Integrated Hardening, Freeze, Final Candidate):** integrated audit of Parts A–D
  ...
  `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md` Part E.
- **MVP-1.3 planning/recovery review (read-only):** produced a complete gap-analysis/architecture
  package (`docs/architecture/VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md`) identifying five
...
- **MVP-1.3-A (Business Identity Foundation) — implemented and complete (2026-08-18).** The five
  ...
  precedent). Full evidence: `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Part A.
...
- **MVP-1.3-B (Profile Presentation & Sharing Foundation) — implemented and complete
  ...
  (deferred to Part C). Full evidence: `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md`
  Part B.
- **MVP-1.3-C (Integrated Hardening, Freeze, Final Candidate) — implemented and complete
  ...
  `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md` Part C.
...
`apps/venture_android/app/src/main/java/com/jajusri/venture/feature/masterdata/ledger/presentation/{LedgerStatementScreen.kt,LedgerStatementUiState.kt,LedgerStatementViewModel.kt}`.
...
shape Phase 3E fixed for Vouchers ([[project_venture_phase3e_offline_voucher]] in memory —
...
available. Connect to VENTURE Desktop and synchronize once." when the company has never been
...
`apps/venture_android/app/src/main/java/com/jajusri/venture/feature/masterdata/ledger/{domain/repository/LedgerRepository.kt,domain/usecase/LedgerUseCases.kt,data/repository/LedgerRepositoryImpl.kt,data/repository/SearchLedgersPortImpl.kt}`.
...
3. `git diff` renders `LedgerStatementScreenTest.kt`'s change as far larger than the actual edit
   ...
   `composeRule.setContent { VentureTheme { LedgerStatementScreen(...) } }` blocks defeat the line
...
`I2407`), against a genuinely live, already-paired environment: VENTURE Desktop + Tally +
...
confirming a genuine update, not a fresh install — app's own `venture.db`/`datastore` preserved
...
warmed into Room, confirmed via direct `sqlite3` inspection of the pulled `venture.db` — all rows
...
`apps/venture_android/app/src/main/java/com/jajusri/venture/feature/masterdata/ledger/data/repository/LedgerRepositoryImpl.kt`
...
`apps/venture_android/app/src/main/java/com/jajusri/venture/feature/sync/presentation/SyncViewModel.kt`
...
`sqlite3` inspection of the Android app's own `venture.db` confirmed `cached_ledgers.parentGroup`
is `NULL` for **all 941** real ledgers. Cross-checked the Desktop Connector's own independent
local cache (`connector-data/venture-ledger.db`, `ledgers.parent_group`) on the same machine: also
...
(`connector/venture_connector/`, distinct from and never touching the separate, standing-instruction-
protected `C:\Projects\Venture_connectivity_hardening\connector\venture_connector\` worktree used for
...
- **F — root cause, found at the request layer:** `MasterDataTemplates.ledgers()`
  (`connector/venture_connector/src/extraction/templates/master-data-templates.ts`)'s
  ...
  `connector-data/venture-ledger.db` (`ledgers.parent_group`) shows `NULL` for all 941 real ledgers —
...
Recorded verbatim as specified, as a new entry in `docs/planning/VENTURE-NOT-NOW.md` ("Prospect →
...
Read and reconciled `VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md`, `VENTURE-PRODUCT-DECISION-LOG.md`
(PDL-001 through PDL-019 in full), `VENTURE-UI-DESIGN-DECISIONS.md`, `VENTURE-SCREEN-INVENTORY.md`,
`VENTURE-NOT-NOW.md`, `VENTURE-MVP-1-3-BUSINESS-PROFILE-ARCHITECTURE.md` (the direct structural
...
**Produced:** `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` — a full planning package
...
`docs/technical-debt/registry.md` (new TD-035 + index entry), `docs/planning/VENTURE-NOT-NOW.md` (new
Prospect→Ledger entry), `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` (new file),
...
1. **MVP-1.4 Brainstorm 1** (User + ChatGPT), using
   `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` §5.3 as the exact input — the nine
...
2. **TD-035 decision** (`docs/technical-debt/registry.md`): re-enable Connector `PARENT` fetch for
   ...
   (`docs/planning/VENTURE-NOT-NOW.md`) is ever scheduled.
...
(`connector/venture_connector/src/extraction/templates/master-data-templates.ts`) — **Ledgers only**.
...
Prospects (a VENTURE-only concept, never Tally-ledger-backed) are not reachable by this path at all, so
...
Added **PDL-020** to `docs/governance/VENTURE-PRODUCT-DECISION-LOG.md`, transcribing this task's nine
...
`docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md`'s header and §5.3 with the same
...
Unchanged this session — the Phase 38 entry in `docs/planning/VENTURE-NOT-NOW.md` was read to confirm
...
implementation prompt using `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md` §5.3 (now
...
already-installed package via `dumpsys package com.jajusri.venture.debug` before installing (versionCode
...
`adb shell monkey -p com.jajusri.venture.debug -c android.intent.category.LAUNCHER 1` (the
`am start -n com.jajusri.venture/.MainActivity` component name used by earlier sessions no longer
resolves; the real activity is `com.jajusri.venture.app.MainActivity` — recorded for future sessions).
Confirmed `mFocusedApp` became `com.jajusri.venture.debug/com.jajusri.venture.app.MainActivity`; zero
...
in `apps/venture_desktop`. Starting HEAD `0d91eac` (Phase 39's push-confirmation commit), working
...
In `startDesktopShell()` (`src/renderer/scripts/app.ts`), the `window.ventureDesktop.onStatusUpdated`
...
`Venture Desktop.exe` (`C:\Program Files\Venture Desktop\`, PID 7000 + GPU/utility/renderer children),
using the same `userData` directory (`%APPDATA%\@venture\desktop`) the dev build would use — the dev
...
`Venture Desktop.exe` instance (pre-existing, pre-fix build, `C:\Program Files\Venture Desktop\`) using
...
(`VENTURE_INSTALLED_PROBE_MODE`/`VENTURE_USER_DATA_DIR`, `src/application/release/startup-environment.ts`,
...
temp root, `VENTURE_SKIP_SINGLE_INSTANCE=true` (no interaction with the production instance's lock),
...
processes, the production `Venture Desktop.exe` instance's four process IDs were byte-identical before
...
All in `apps/venture_desktop/src/renderer/scripts/app.ts` unless noted, reusing existing components/
...
Reused the isolated `VENTURE_INSTALLED_PROBE_MODE` probe methodology from the Phase 40 follow-up
(own `mkdtemp`'d `userData` dir, ephemeral connector port, `VENTURE_SKIP_SINGLE_INSTANCE=true`) — the
...
(`connector/venture_connector/src/services/health/health-service.ts`: `status = 'degraded'` whenever
...
Reused the isolated `VENTURE_INSTALLED_PROBE_MODE` probe methodology from Phase 40/41 (own `mkdtemp`'d
`userData` dir, ephemeral connector port, `VENTURE_SKIP_SINGLE_INSTANCE=true`) against the same live
...
VENTURE is ready, whether Tally is connected, which company they're working with, whether data is
...
- A general-purpose research agent traced the Connector's sync endpoints, extraction pipeline,
  ...
  semantics directly from `connector/venture_connector/src`.
- A second traced Android's sync triggers, Room caching, company-scoped state, local-first behavior,
  ...
  `apps/venture_android/app/src/main/java/com/jajusri/venture`.
...
New document: `docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md` (19 sections, matching
...
- Current checkpoint: `docs/status/VENTURE-CURRENT-DEVELOPMENT-STATUS.md`
- MVP-1.1 Connect/Universal Party: `docs/status/VENTURE-MVP-1-1-CONNECT-STATUS.md`
- MVP-1.2 Relationship Timeline/Dincharya: `docs/status/VENTURE-MVP-1-2-RELATIONSHIP-TIMELINE-DINCHARYA-STATUS.md`
- MVP-1.3 Business Profile: `docs/status/VENTURE-MVP-1-3-BUSINESS-PROFILE-STATUS.md`
- MVP-1.4 Catalogue (planning only, not authorized): `docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md`
- Controlled Pilot: `docs/planning/VENTURE-MVP-1-CONTROLLED-PILOT-CLOSURE-STATUS.md`
- Quality: `docs/governance/VENTURE-QUALITY-SCORECARD.md`
- Technical debt: `docs/technical-debt/registry.md`
- UI/UX: `docs/design/VENTURE-MVP-1-UI-UX-POLISH-STATUS.md`
- Refinement/physical acceptance: `docs/planning/VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md`
- Scope/roadmap: `docs/planning/VENTURE-MASTER-PRODUCT-EXECUTION-PLAN.md`, `docs/ROADMAP.md`
...
`origin/main`. Continuation of [[project_venture_td035_physical_validation]]'s deferred physical
...
(the installed production build already running in `C:\Program Files\Venture Desktop`, bound
...
1. **The Sync screen's "Sync Now" does not populate Android's Room cache.** It only tells the
   ...
   validated tap sequence" in [[project_venture_td035_physical_validation]] had not accounted for.
...
`docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md` exactly:
...
A standalone build of the new Connector (dist, port 8090, isolated database, `VENTURE_DATABASE_PATH`
...
`StartTargetSyncUseCase` (`apps/venture_android/.../sync/domain/usecase/SyncUseCases.kt`) gained a
...
equivalent Stock Item fingerprint — confirming VENTURE's *existing* definition of "change" already
...
open). The request VENTURE's own dead-code scaffold would have sent — `<TYPE>Object</TYPE>
...
VENTURE's own schema: `idx_ledgers_company_guid`/`idx_stock_items_company_guid` in
`connector/venture_connector/src/storage/sqlite/schema.ts` are already unique on `(company_id, guid)`,
...
evidence recorded in `docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md` §3.1, and its
...
untested request shapes were attempted afterward — only requests already proven safe by VENTURE's own
...
VENTURE's Connector entirely** — the Connector process was not even running at the time (confirmed:
no `node.exe`/`electron.exe` processes present). It was not, at the moment it was sent, a real
VENTURE production request.

**However, this same investigation discovered that the exact same broken shape *is* independently
constructed by real, "production"-classified VENTURE Connector code** — see §B below (**TD-040**).
...
independent of any assumption about VENTURE.

**Answering the forensic questions directly:**
- Could VENTURE-side limits have been exceeded? No — this bypassed the Connector, so none of its
...
- Evidence of VENTURE accessing native memory, pointers, or unsafe APIs? None. The client was a plain
...
- **CAUSE (exact internal mechanism) NOT PROVEN** in the strict sense: no Tally crash-dump or stack
  ...
  is stated above. What **is** proven: this was not a VENTURE production request at the time it was
  sent (Connector wasn't running), it is not evidence of any VENTURE-side memory-unsafe code, and the
  request was objectively structurally invalid per Tally's own documented contract.

### B. TD-040 — a real, live, "production" VENTURE defect discovered by this same review

The forensic review's evidence-gathering step (checking whether "any existing VENTURE request could
reproduce the problem") found that it could: `ApprovedOperationId.CompanyInfo`
(`connector/venture_connector/src/tally/registry/operation-registry.ts`, `rolloutStatus:
...
`docs/architecture/VENTURE-ADAPTIVE-TALLY-SYNC-ARCHITECTURE.md` §3.1 already recorded the immediate
...
(`docs/architecture/VENTURE-MVP-1-1-CONNECT-UNIVERSAL-PARTY-ARCHITECTURE.md` §8): a single canonical
...
shortcut concept. New pure classifier `com.jajusri.venture.core.util.AliasSearchClassifier
...
- **"View Vouchers" seeds a free-text party-name filter, not a stable ledger-id filter** — a
  ...
  `VENTURE-MVP-1-1-CONNECT-STATUS.md`) that would require Voucher-schema changes outside Connect's
...
`VENTURE-NOT-NOW.md`). No new Tally request of any kind — this entire phase touched only Android UI/
...
`connectedProdDebugAndroidTest` uninstalled the `com.jajusri.venture.debug` package as part of its
...
`apps/venture_desktop`, which builds and launches the real Electron Desktop app that manages the
...
GUI-free Connector process (`npm run dev` in `connector/venture_connector`) was considered as an
...
uninstalled `com.jajusri.venture.debug` as Gradle's own managed lifecycle — reinstalled from the
...
Traced to source: the real Desktop app (`apps/venture_desktop`) stores its config at
`%APPDATA%\@venture\desktop\desktop-config.json` (`connectorBindMode: "trusted-lan"`, `connectorHost:
...
query against the live `databases/venture.db` at capture time confirmed 926 `cached_parties` rows for
...
message shown is **"No offline data available. Connect to VENTURE Desktop and synchronize once."** —
...
`adb shell run-as com.jajusri.venture.debug sqlite3 databases/venture.db` against the real, live
...
(`connector/venture_connector/src/extraction/templates/master-data-templates.ts`) — the Ledgers TDL
...
`E:\VenturePrivate\...\connector-data\venture-ledger.db` — not the stale `%APPDATA%` copy from
...
scope (`docs/architecture/VENTURE-MVP-1-4-CATALOGUE-ARCHITECTURE.md`, `...BRAINSTORM-OUTCOME.md`) —
...
`Routes.CATALOGUE`/`Routes.CATALOGUE_DETAIL` in `VentureNavHost`.
...
`com.jajusri.venture.debug` install** (real company ESTIMATION, "Tally connected" shown, real
...
(`venture-desktop.log`, timestamps matching this exact session) showed repeated
...
installed the **Prod-flavor** debug variant (`installProdDebug` — `com.jajusri.venture.debug`, the
package that actually carries this device's real pairing/data; Phase 61's `installDevDebug` had
targeted the empty `com.jajusri.venture.dev.debug` sibling package by mistake, caught and corrected
...
after discovering the correct file is `venture.db`, not the empty stray `venture_database` file also
...
by the governing task against `docs/architecture/VENTURE-MVP-1_4-CATALOGUE-BRAINSTORM-OUTCOME.md`
...
- **TD-044** (no real owner/role identity anywhere in VENTURE) — pre-existing, cross-cutting, out of
...
(`docs/architecture/VENTURE-TRANSACTION-MODE-ARCHITECTURE.md`) against the locked Q1–18 Brainstorm
...
- **Accounting-software intelligence boundary** (Phase 6) — no code change needed: `findCompletedPurchaseHistory`/
  `BuyAgainListBuilder` already only ever read VENTURE's own synced transaction data (never
...
in the existing `VentureNavHost`, and one new entry point: a `ShoppingCart`-icon `IconButton` added
...
`apps/venture_android/app/build/outputs/apk/dev/debug/app-dev-debug.apk` (~15.6 MB, built
...
`apps/venture_android/app/build/outputs/apk/dev/debug/app-dev-debug.apk` (freshly timestamped,
...
build over the user's own already-paired, already-synced device install (`com.jajusri.venture.debug`
...
1. **WhatsApp share was completely broken** — tapping Share always failed with "The share file
   ...
   file (`<uuid>-venture-estimate.txt`, 279 B) and WhatsApp genuinely listed as a target — did not tap
