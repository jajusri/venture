...
| Upgrade/release readiness | 2 | **Updated 2026-08-15, second pass.** A current-HEAD controlled-pilot candidate now exists for both components with full manifest/checksum/provenance validation: Desktop `0.4.8` (`BudcomDesktop-0.4.8-x64-setup.exe`, SHA-256 `61efcd6...`, commit `b2023df`) and Android `continuity.15`/versionCode 16 debug APK (SHA-256 `7a67db1...`, commit `8808e76`). Raised from 1 because "no artifact exists" is no longer true. Capped at 2, not higher, because: neither artifact has been installed on real hardware yet; Android has no signed-release path at all (no keystore in this environment); and the lifecycle-gate lifecycle report from the prior era remains the only one on file. |
...
**Weakest dimension:** Physical validation (1) — a current-version distributable now exists (Upgrade/release readiness raised to 2, 2026-08-15) but has not been physically installed or tested on real hardware.
**Required correction before release:** Complete the five ordered human-check sessions in `docs/planning/BUDCOM-MVP-1-CONTROLLED-PILOT-CLOSURE-STATUS.md` §11 against the now-produced Desktop `0.4.8` / Android `continuity.15` candidates (Windows install/restart/reconnect, Android upgrade/reconnect/voucher-sync, private-storage recovery). Re-score this document once that evidence lands.
