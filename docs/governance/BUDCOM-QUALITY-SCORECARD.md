...
| Connectivity/recovery | 2 | **Updated 2026-08-16 (autonomous hardening run).** Automated fixes exist for TD-013–TD-017 plus TD-029 (Desktop stayed bound to a departed network's IP) and TD-030 (mDNS advertised unreachable IPv6 addresses), both found during Session 3 item R's physical retest chain, both root-caused and fixed, both with new regression coverage, full suites clean. Still capped at 2, not higher: the combined TD-029+TD-030 fix has not yet had its physical retest (the exact scenario that found both defects) — recorded PENDING HUMAN CONFIRMATION, not yet run. TD-017's physical retest remains blocked pending that same retest; TD-013's restart-recovery scenario is still unconfirmed. |
...
| UX clarity | — **(judgment)** | **Updated 2026-08-16.** Session 2's full Android core-workflow set (sync, Ledger Browser, Voucher Browser + type coverage, Voucher/Ledger PDF generation, WhatsApp share/save) has now been physically exercised end to end and passed — this dimension has real human-exercised evidence for that scope, though still not a full product-owner UX review. One new gap found and explicitly deferred: PDF cannot be previewed before the WhatsApp/share action (TD-028, P3, post-MVP-1). Pre-existing gap unchanged: private-storage mid-session loss shows a generic "Disconnected" state rather than a storage-specific one (TD-025). |
...
| Upgrade/release readiness | 2 | **Updated 2026-08-16 (autonomous hardening run).** Desktop 0.4.12 physically confirmed Session 2 in full (see Physical validation row). Current candidate: Desktop `0.4.14` / Connector `0.4.5`, carrying both TD-029 and TD-030 (closure doc §35-§37 for full evidence and exact artifact/SHA-256/commit). Android `continuity.15`/versionCode 16 debug APK (SHA-256 `7a67db1...`, commit `8808e76`) is unchanged — both fixes are Desktop/Connector-only. Still capped at 2, not higher: Android has no signed-release path at all (no keystore in this environment); 0.4.14 has not yet been physically retested; Sessions 1/4/5 haven't run, and Session 3 is paused pending the combined TD-029+TD-030 physical retest. |
...
**Required correction before release:** Physically retest TD-029's fix (Session 3 item R — real network-address change) against Desktop `0.4.13`, then complete the rest of Session 3 plus Sessions 1, 4, 5 in `docs/planning/BUDCOM-MVP-1-CONTROLLED-PILOT-CLOSURE-STATUS.md` §11. Re-score this document once that evidence lands.
**Accepted limitations:** Private-storage hot-removal mid-write atomicity not fully guaranteed (documented); TD-022/TD-023 performance observations (P3, non-blocking); TD-025 storage-loss UX gap (P2, open, fails closed/safely); TD-026 out-of-window Voucher carry-forward has no re-verification against Tally (P2, deferred); TD-027 Tally Export-interface can temporarily lag its own UI/reports for very recently entered vouchers (P2, observational, not a BUDCOM defect); TD-028 Voucher/Ledger PDF cannot be previewed before share (P3, deferred to post-MVP-1 by explicit user decision — not reopened by the 2026-08-16 autonomous hardening run); TD-029 Desktop network-rebind fix and TD-030 mDNS over-advertisement fix both implemented and automated-validated, physical retest of the combined scenario still pending (P0, this is the current release blocker).
