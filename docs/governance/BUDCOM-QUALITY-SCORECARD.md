...
| Connectivity/recovery | 2 | **Updated 2026-08-16 (round 4 of the same Session 3 item R retest chain).** Automated fixes exist for TD-013–TD-017 plus TD-029 (Desktop stayed bound to a departed network's IP), TD-030 (mDNS advertised unreachable IPv6 addresses), TD-031 (mDNS never advertised the HTTPS/authenticated port, so TD-017's rediscovery mechanism verified TLS against the wrong port), and now **TD-032** (on BBK/vivo-family Android devices, the OS's own Wi-Fi driver silently drops all mDNS multicast traffic unless the app holds a `WifiManager.MulticastLock`, which the app never did — proven live: `ip maddr` polled at 300ms resolution across multiple discovery windows on the actual physical retest device never showed the mDNS multicast group joined, even with TD-031's fix independently reconfirmed correct via a live mDNS browse taken at the exact moment of failure). All five are root-caused and fixed with new regression coverage; full suites clean (Connector 1,419/1,419, Android debug+release 1,021/1,021 each, 0 lint issues). TD-032 retroactively explains why the physical retest still failed even after TD-031's fix was proven correct — a defect layered underneath everything TD-017/TD-031 could ever have controlled, not a flaky or partial result. Still capped at 2, not higher: the combined TD-029+TD-030+TD-031+TD-032 fix has not yet had its physical retest (the exact A→B→A scenario that found all of them) — recorded PENDING HUMAN CONFIRMATION, not yet run. TD-013's restart-recovery scenario is still unconfirmed. |
...
| Upgrade/release readiness | 2 | **Updated 2026-08-16 (TD-032 candidate).** Desktop 0.4.12 physically confirmed Session 2 in full (see Physical validation row). Current candidates: Desktop `0.4.15` / Connector `0.4.6` (unchanged since TD-031, closure doc §35-§40) SHA-256 `321c0946ff2fc263a5558bf55cc34886f7358bbf5a7eb98da674d5a6144899b5`, and Android `continuity.18`/versionCode 19 debug APK (TD-032, closure doc §41), SHA-256 `4c84276cdc1d2ac4160ae1eddbfb1ee17d7550757cb84dd3335111664226cdfe` — Android-only this round; Desktop/Connector genuinely unchanged, not merely re-labeled. Still capped at 2, not higher: Android has no signed-release path at all (no keystore in this environment); the new candidates have not been physically retested yet; Sessions 1/4/5 haven't run, and Session 3 is paused pending the combined TD-029+TD-030+TD-031+TD-032 physical retest. |
