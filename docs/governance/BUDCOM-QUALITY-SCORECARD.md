...
| Connectivity/recovery | 2 | **Updated 2026-08-16 (round 3 of the same Session 3 item R retest chain).** Automated fixes exist for TD-013–TD-017 plus TD-029 (Desktop stayed bound to a departed network's IP), TD-030 (mDNS advertised unreachable IPv6 addresses), and now **TD-031** (mDNS never advertised the HTTPS/authenticated port at all, so TD-017's rediscovery mechanism verified TLS against the wrong port on every single attempt — proven live via curl: the advertised port failed the TLS handshake instantly while the actual secure port answered healthy in ~13ms). All four are root-caused and fixed with new regression coverage; full suites clean (Connector 1,419/1,419, Android debug+release 1,017/1,017 each, 0 lint issues). TD-031 retroactively explains why TD-017's physical retest had never once succeeded across the entire engagement — it was structurally impossible before this fix, not a flaky or partial result. Still capped at 2, not higher: the combined TD-029+TD-030+TD-031 fix has not yet had its physical retest (the exact A→B→A scenario that found all of them) — recorded PENDING HUMAN CONFIRMATION, not yet run. TD-013's restart-recovery scenario is still unconfirmed. |
...
| Upgrade/release readiness | 2 | **Updated 2026-08-16 (TD-031 candidate).** Desktop 0.4.12 physically confirmed Session 2 in full (see Physical validation row). Current candidates: Desktop `0.4.15` / Connector `0.4.6` (TD-029+TD-030+TD-031 combined, closure doc §35-§40), SHA-256 `321c0946ff2fc263a5558bf55cc34886f7358bbf5a7eb98da674d5a6144899b5`, and Android `continuity.17`/versionCode 18 debug APK, SHA-256 `28845dedd2d1bfd6023ad811326707a646c66c041e3c2007b37344f24b42b3db` — both bumped together this round since TD-031 required coordinated Connector+Android changes. Still capped at 2, not higher: Android has no signed-release path at all (no keystore in this environment); the new candidates have not been physically retested yet; Sessions 1/4/5 haven't run, and Session 3 is paused pending the combined TD-029+TD-030+TD-031 physical retest. |
