...
| Connectivity/recovery | 2 | **Updated 2026-08-16.** Automated fixes exist for TD-013–TD-017 plus the newly found and fixed TD-029 (Desktop trusted-LAN Connector stayed bound to a departed network's IP after a real network change — found during Session 3 item R's physical retest, root-caused, fixed, 20 new/updated regression tests, full suites clean). Still capped at 2, not higher: TD-029's own physical retest (the exact scenario that found it) has not yet run against the fix, TD-017's physical retest remains blocked pending that same retest, and TD-013's restart-recovery scenario is still unconfirmed. |
...
| Upgrade/release readiness | 2 | **Updated 2026-08-16.** Desktop 0.4.12 physically confirmed Session 2 in full (see Physical validation row). A new candidate, Desktop `0.4.13`, is being produced to carry the TD-029 network-rebind fix found during Session 3 item R — see closure doc §35/§36 for exact artifact/SHA-256/commit once produced. Android `continuity.15`/versionCode 16 debug APK (SHA-256 `7a67db1...`, commit `8808e76`) is unchanged — this fix is Desktop-only. Still capped at 2, not higher: Android has no signed-release path at all (no keystore in this environment); 0.4.13 has not yet been physically retested; Sessions 1/4/5 haven't run, and Session 3 is paused pending TD-029's physical retest. |
