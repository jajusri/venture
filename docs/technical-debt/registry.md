...
| **Status** | **Fixed (automated validation) — physical confirmation pending.** Architectural decision approved 2026-08-16: sanitize XML-1.0-illegal C0 control characters (literal bytes and numeric character references, including `&#4;`) at the shared parsing boundary, before structural parsing, rather than rejecting the whole response. Implemented in `connector/budcom_connector/src/tally/xml/response-parser.ts` (`sanitizeXml10IllegalCharacters()`), applied uniformly to every Tally collection (Groups, Ledgers, Stock items, Vouchers) since it lives in the one shared `TallyXmlResponseParser.parse()` entry point. Structural XML validation (envelope/body/data/collection presence, unclosed tags, nesting/node-count limits) remains fully strict and unchanged — only the character-legality check changed from reject to sanitize. Diagnostic hardening also implemented: `VoucherSynchronizationResult` now carries a `failureDetail` field (the granular reasonCode, e.g. `malformed-xml`/`missing-envelope`/`voucher-ledger-validation`) alongside the existing coarse `failureReason` bucket (`parser_failure` etc.), and `illegalCharactersSanitized` (a count only, never content) — both persisted in the connector's completion log and returned additively in the `POST /sync/vouchers` response (`progress.lastErrorDetail`), without changing the existing `lastError` contract. **Likely side effect (not separately verified this session):** this probably also resolves the original Groups symptom described above — since `&#4;` is now stripped from the `PARENT` field's text entirely, the resulting value should already equal the hierarchy-validator's expected normalized `"Primary"` string without requiring a separate business-logic fix there. Recommend a Groups-specific regression test or physical retest to confirm and formally close that half of this entry. |
| **Evidence** | `docs/testing/milestone-3c-groups-manual-validation.md`; bug ID M3C-001 in `docs/stage-updates/milestone-3c-stage-update.md`; controlled-pilot Session 2 physical failure (2026-08-16, ESTIMATION, Android Sync screen: `Vouchers - Failed, Processed 0, Error: parser_failure`, Ledgers/Stock items both Completed in the same run); fix validated by regression tests in `test/unit/tally/response-parser-limits.test.ts` (sanitizer contract: class-based C0 coverage, legal-whitespace preservation, ordinary-Unicode non-interference, structural-strictness-after-sanitization), `test/unit/voucher/voucher-parser.test.ts` (TD-001 artifact in a Voucher's `PARTYLEDGERNAME` now parses successfully), `test/unit/voucher/voucher-snapshot-sync.test.ts` (`failureDetail`/`illegalCharactersSanitized` threading, all seven granular reasonCodes), `test/unit/extraction/master-data-templates.test.ts`, and `test/unit/voucher/two-phase-voucher-extraction-design.test.ts`. Full connector suite: 157 files / 1377 tests passing, `eslint`/`tsc --noEmit`/build/architecture tests all clean. Desktop (566→677 tests, unaffected, no source changed) and contract suites (5 tests) re-confirmed unaffected. |
...
| **Required physical confirmation** | Fresh Voucher sync against ESTIMATION on a new Desktop candidate bundling this fix, confirming Vouchers now completes instead of failing with `parser_failure`. |
...
| TD-001 | Tally `&#4;` illegal-character export artifact — shared-parser sanitizer implemented, Voucher sync fix confirmed by automated tests | **P0** | **Fixed (automated validation) — physical confirmation pending** | Pre-MVP-1 release hardening |
