...
| TD-001 | Tally Voucher sync `parser_failure` — root cause narrowed to `VoucherLedgerEntryParser` (0.4.9 and 0.4.10 both FAILED); illegal-char and two-phase-deletion hypotheses ruled out by audit evidence; parser now typed with 13 distinguishable reasons, next retest (0.4.11) must read `parseReason` before any fix | **P0** | **OPEN — root cause narrowed, diagnostic round 3 awaiting physical retest** | Pre-MVP-1 release hardening |
