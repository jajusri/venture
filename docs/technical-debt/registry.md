...
| **Required physical confirmation** | One fresh Voucher sync against ESTIMATION on the next Desktop candidate (`0.4.12`; Android `continuity.15` unchanged). Expected: sync completes, and if the audit file records anything it should show a *successful* run, not a failure. Report voucher counts, any `amountSignConflictCount` > 0 observed, and whether existing voucher data/totals remain correct. |
