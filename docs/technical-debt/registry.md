...
| TD-001 | Tally Voucher sync `parser_failure` — illegal-character sanitizer shipped but physical retest (0.4.9) still FAILED; file-based failure auditor + deletion-reconciliation hypothesis added, next retest must read the audit file before further guessing | **P0** | **OPEN — sanitizer insufficient, diagnostic round 2 awaiting physical retest** | Pre-MVP-1 release hardening |
