# VENTURE — Connect Contacts, Universal Party Identity & Referral Tree Specification
...
It is VENTURE's **Business Relationship Record** around one canonical party identity.

For every party, VENTURE should help answer:
...
8. Is this party on VENTURE and connected to us?
...
One real business/person must resolve to one stable VENTURE Party Identity.
...
The Party Identity is the anchor. Ledger/accounting data, mobile/contact data, referral ancestry, VENTURE network identity and later profile/catalogue/message relationships attach to it.
...
- VENTURE account/business identity;
...
- Tally/VENTURE ledger contacts;
- VENTURE-created Prospects.

The device phonebook is **not** an uncontrolled canonical import source in the initial implementation.

If device-contact matching is introduced later, it must be a deliberate enrichment/matching workflow around Universal Party Identity, not a dump of personal contacts into VENTURE.
...
- **Prospects:** VENTURE-created parties not yet represented by an accounting customer ledger.

Do not make "VENTURE" a mutually exclusive category beside Customer/Prospect because a party may simultaneously be a Customer and a VENTURE-connected party.

### 4.2 Separate VENTURE connection-state filter

Provide a secondary filter/state such as:

**All | On VENTURE | Connected**

Internally support clear connection states:

- Not on VENTURE
- VENTURE identity found
- Connection pending
- Connected
- Disconnected
- Blocked
...
Tags remain VENTURE data unless a later explicit product decision says otherwise.
...
- VENTURE connection indicator if applicable
...
- Call
- WhatsApp
- Vartalap / VENTURE Message when available
...
VENTURE must allow gradual contact enrichment.
...
VENTURE-only enrichment may also exist where appropriate.
...
- **Black:** added/edited in VENTURE but not yet confirmed back from Tally.
...
- later VENTURE-network verified
...
VENTURE must not directly write into Tally.

Locked workflow:

**Tally → VENTURE sync → user enriches/edits → review → generate Tally-compatible XML → user imports XML into Tally → VENTURE re-syncs → matching values become Green**

No TDL dependency.

VENTURE-only fields do not automatically go back to Tally.
...
Additional people may remain VENTURE-only.
...
If a Prospect later becomes an accounting customer/ledger, VENTURE must propose linking the accounting identity to the existing Party Identity.
...
→ VENTURE suggests a possible identity match  
→ user confirms  
...
VENTURE should ultimately distinguish:
...
VENTURE measures and explains; it does not silently create financial obligations or entitlement to rewards.
...
Notes remain VENTURE data unless explicitly included in a future shared workflow.
...
Internal relationship data must not leak merely because two businesses connect on VENTURE.
...
If VENTURE opens WhatsApp, it may record:

`WhatsApp opened/shared from VENTURE`
...
## 23. Handoff to future VENTURE modules
...
### Suggested Actions
What VENTURE recommends considering.
...
- view/edit VENTURE-side fields;
- notes;
- tags;
- Referral Tree already stored locally;
- prepare pending work for later synchronization where safe.

Network-specific VENTURE connection/profile/Vartalap features may show a clear unavailable/stale state when offline.
...
3. Never expose private internal fields through a VENTURE connection by default.
...
4. VENTURE connection state is separate from commercial classification.
...
14. Contact fields may be enriched in VENTURE.
15. Tally-confirmed vs pending VENTURE fields remain distinguishable.
