# BUDCOM MVP-1 Public Release Gate Matrix
...
`docs/planning/BUDCOM-MVP-1-CONTROLLED-PILOT-CLOSURE-STATUS.md` (pilot-scoped) and
`docs/governance/BUDCOM-QUALITY-SCORECARD.md` (dimension scoring) — this document classifies each
...
| C | Windows installer | **MUST FIX → RESOLVED this session (TD-033) / SHOULD FIX remainder** | NSIS installer (`electron-builder.yml`) is per-machine, `deleteAppDataOnUninstall: false`, custom `installer.nsh` migrates legacy transport identity safely with abort-on-conflict. Product name/publisher/icon already set (`Budcom Desktop`, copyright `Budcom`). Storage-mode switch guard (TD-033) and unreadable-marker fail-closed (TD-034) implemented and tested this session — see §5/§6. Remaining: `appId: com.budcom.desktop` / `productName: Budcom Desktop` vs. the in-app title still reading "Business OS Tally Connector" in places (pre-existing, flagged, not renamed — see prior UI/UX status doc §7, a naming decision, not a defect). |
| D | Android package/artifact | **MUST FIX identified, BLOCKED on §A** | `assembleRelease` clean; `applicationId = "com.budcom.android"` (debug variant appends `.debug`). Public Play-Store-facing applicationId is not formally decided anywhere in governance docs — see §13 below, flagged as a release blocker requiring a product decision, not invented here. |
...
| V | Release notes | **RESOLVED this session** | `docs/planning/BUDCOM-MVP-1-RELEASE-NOTES.md` (new, this session). |
| W | Rollback/supportability | **SHOULD FIX — guidance added, tooling unchanged** | `docs/planning/BUDCOM-MVP-1-QUICK-START.md` (new) includes basic troubleshooting; no dedicated rollback tooling exists beyond "reinstall the previous installer" (acceptable for an initial limited release, not for broad public support). |
...
- Nothing is deleted. The prior vault/locator record is left exactly as it was; only the *pointer*
  to what BUDCOM currently reads changes, once confirmed.
...
BOM-prefixed write from a non-BUDCOM tool) identically to "no marker at all," so the adoption path
...
controlled-pilot operating constraint: do not remove/reconnect private storage while BUDCOM is
running; if the drive needs to change, stop BUDCOM first. This is now stated plainly in the
...
2. Add a `signingConfigs { release { ... } }` block to `apps/budcom_android/app/build.gradle.kts`
...
`com.budcom.android` is the current base id (release), `com.budcom.android.debug` for debug
...
- `docs/planning/BUDCOM-MVP-1-RELEASE-NOTES.md` — user-facing release notes.
- `docs/planning/BUDCOM-MVP-1-QUICK-START.md` — install/first-use guide plus basic troubleshooting.
- This document.
...
|---|---|---|---|---|---|
| Android (debug, previously installed/approved) | `0.1.1-continuity.21`, versionCode 22 | `bc9cd55` | `release/controlled-pilot/android/0.1.1-continuity.21-bc9cd55/BudcomAndroid-bc9cd55-debug.apk` | `bb5ece6bc93383f93cf16b670f8506967e85295ecf249b97e10bd944074705c8` | Debug keystore only |
| Android (release, unsigned, this session) | `0.1.1-continuity.21`, versionCode 22 | `2a35e25` | `apps/budcom_android/app/build/outputs/apk/release/app-release-unsigned.apk` | see final report | **UNSIGNED — NOT FOR DISTRIBUTION** |
