# VENTURE Master Product Execution Plan
...
This file converts the VENTURE roadmap into an executable development
...
Numerical forecasts must be calibrated primarily from actual VENTURE
...
Tracked in `VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md`, which holds the
...
-   VENTURE does not directly write to Tally; user-initiated compatible
...
-   Call / VENTURE message / WhatsApp actions;
-   View Ledger / View Vouchers;
-   VENTURE enrichment → Tally-compatible XML → user import → re-sync
    confirmation;
...
Locked visual/ownership shape: `docs/design/VENTURE-UI-DESIGN-DECISIONS.md`
...
as MVP-1.3 — see `docs/design/VENTURE-UI-DESIGN-DECISIONS.md` §7. It does not
...
actual VENTURE runs provide evidence.
