# VENTURE MVP-1 Controlled-Pilot Closure Status
...
`docs/technical-debt/registry.md`, `docs/planning/VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md`,
governance docs, and `C:\Projects\Venture_archives` physical-evidence files.
...
|---|---|---|
| `apps/venture_desktop/package.json` | Uncommitted, unstaged change strips the entire `scripts` block (`build`/`start`/`test`/`lint`/`dist:win`/`release:controlled-pilot`/`screenshot`) and all `devDependencies`, and reverts `version` 0.4.7→0.4.6. File mtime (2026-08-12 08:32) predates the last 5 commits — this sat broken, uncommitted, unnoticed through several commits. This is **corruption, not intentional work**: no legitimate edit both removes build tooling and decrements a version. `npm run build`/`test`/`lint`/`dist:win`/`release:controlled-pilot` would not resolve from this file as-is. | **OPEN → fixed this session** (restored from HEAD; see §7) |
...
| `docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md`, `docs/product/`, `docs/product-design/` (untracked) | Post-MVP-1 Connect/UX planning material (docx specs, implementation checklists). Explicitly out of MVP-1 scope per this task's mandate. | **Left untracked, not actioned** — informational only |
| Worktree `C:\Projects\Venture_connectivity_hardening` on branch `post-mvp1/secure-local-pairing` (commit `8461901`, diverged/unmerged from `main`) | Separate, correctly isolated post-MVP-1 secure-pairing work in its own worktree. Not polluting `main`. | **Not touched** — out of scope |
| Branch `release/desktop-0.4.3-connector-0.4.0`, and ~14 detached-HEAD build worktrees under `D:/tmp/venture-release-*` | Disposable release-build worktrees from the `controlled-pilot-release.mjs` pipeline, at various historical commits. Not part of `main`'s working-tree status. | **Not touched** — housekeeping only if requested |
...
cross-reference against `C:\Projects\Venture_archives` (which has **zero files dated
...
(`VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md` §9, 2026-08-13) produced a materially more
...
- `docs/governance/VENTURE-QUALITY-SCORECARD.md`, which
  `VENTURE-MVP-1-0-X-REFINEMENT-REGISTER.md` §7 makes a **mandatory** MVP-1 freeze gate,
...
1. Restored `apps/venture_desktop/package.json` to its committed HEAD content (corruption
...
|---|---|---|---|
| Desktop | `0.4.8` | `33f6edd` | `0.4.7` predated this session's TD-024 fix landing in `apps/venture_desktop/src` — packaging under the stale label would misrepresent the candidate's contents |
...
(`docs/planning/VENTURE-CONNECT-CONTACTS-UNIVERSAL-PARTY-REFERRAL-TREE-SPEC.md`,
...
**Artifact:** `release/controlled-pilot/0.4.8/artifacts/VentureDesktop-0.4.8-x64-setup.exe`
...
per `C:\Projects\Venture_archives\temporary-builds\20260808_phase3u6\VentureAndroid-d76b3d0-debug.apk`,
...
**Artifact:** `release/controlled-pilot/android/0.1.1-continuity.15-8808e76/VentureAndroid-8808e76-debug.apk`
...
environment (`apps/venture_android/app/build.gradle.kts`'s `release {}` block has no
...
Updated `docs/governance/VENTURE-QUALITY-SCORECARD.md`:
...
framing point that changes the real risk level:** VENTURE's Connector is architecturally
**read-only against Tally** (`connector/venture_connector/src/tally/security/capabilities.ts`
...
business's real, authoritative accounting records — is never written to by VENTURE and
...
atomicity is not fully guaranteed" (`docs/governance/VENTURE-BACKUP-RECOVERY-AND-DISASTER-RECOVERY.md`
...
2. **Before pulling the drive**, copy the entire `<driveLetter>\VenturePrivate\` folder
...
5. **STOP condition:** if the integrity check fails, or Ledger/Voucher data looks
   ...
   `VenturePrivate` folder), re-run the integrity check to confirm recovery, and report
...
- Expected: No `connector-data\venture-ledger.db` appears under Desktop's AppData path if
  the install is configured for Private storage — check the folder directly if unsure
  where AppData is (`%APPDATA%\venture-desktop\connector-data` or similar).
...
- Desktop `0.4.8` — `release/controlled-pilot/0.4.8/artifacts/VentureDesktop-0.4.8-x64-setup.exe`, SHA-256 `61efcd64bface65fa860933ff0aa76abd6f58fc50cbfdfa043df89f39d4af043`, commit `b2023df`
- Android `0.1.1-continuity.15` (versionCode 16) — `release/controlled-pilot/android/0.1.1-continuity.15-8808e76/VentureAndroid-8808e76-debug.apk`, SHA-256 `7a67db16cf588c57b2b193353e063fe5b98965f65e63e8cbf277b53f1f45ac08`, commit `8808e76`
...
1. `TallyVoucherExtractor.readVouchers()` (`connector/venture_connector/src/tally/voucher/voucher-extractor.ts`)
...
3. `VoucherSynchronizationService.synchronize()`'s catch block
   (`connector/venture_connector/src/services/voucher/voucher-snapshot-sync.service.ts`)
...
(`connector/venture_connector/src/tally/xml/response-parser.ts`) explicitly detects and
...
**Implemented in `connector/venture_connector/src/tally/xml/response-parser.ts`:**
...
`connector/venture_connector` into the installer) — since Connector source changed, the
...
(`release/controlled-pilot/android/0.1.1-continuity.15-8808e76/VentureAndroid-8808e76-debug.apk`)
...
provenance check: `apps/venture_desktop/build/VERSION.txt` — a git-tracked file Desktop
...
**Artifact:** `release/controlled-pilot/0.4.9/artifacts/VentureDesktop-0.4.9-x64-setup.exe`
...
(`connector/venture_connector/src/services/voucher/voucher-sync-failure-audit.ts`)
...
`connector/venture_connector/src/bootstrap/register-services.ts`'s
...
to the spawned Connector child via `VENTURE_VOUCHER_SYNC_FAILURE_AUDIT_PATH`, added to
...
- A **ledger or inventory entry present in a later phase but referencing a GUID absent
  ...
  (`connector/venture_connector/src/tally/voucher/voucher-reconciliation-error.ts`) with
...
`connector/venture_connector/src/tally/voucher/voucher-request.ts:74-140`) do not reuse
...
present in the compiled `connector/venture_connector/dist/` output — the exact artifact
...
**Artifact:** `release/controlled-pilot/0.4.10/artifacts/VentureDesktop-0.4.10-x64-setup.exe`
...
userData root
(`C:\Users\Vidhi\AppData\Roaming\@venture\desktop\connector-diagnostics\voucher-sync-failure-audit.jsonl`)
...
(`connector/venture_connector/src/tally/voucher/voucher-ledger-parser.ts`) — which, at
...
(`connector/venture_connector/src/tally/voucher/voucher-entry-parse-error.ts`) with one
...
**Artifact:** `release/controlled-pilot/0.4.11/artifacts/VentureDesktop-0.4.11-x64-setup.exe`
...
**Artifact:** `release/controlled-pilot/0.4.12/artifacts/VentureDesktop-0.4.12-x64-setup.exe`
...
database (`connector-data/venture-ledger.db`, `node:sqlite` `DatabaseSync` opened with
...
- `docs/governance/VENTURE-QUALITY-SCORECARD.md`: "Physical validation" raised from 1,
...
**Trigger.** Immediately after the §28 physical PASS, the user reported Mobile VENTURE
...
read-only: no new VENTURE sync was triggered, no Tally data was altered, no VENTURE data
...
VENTURE's own discovery phase builds (`buildVoucherCollectionRequestSpec`) directly to
Tally's export port, entirely bypassing the Connector's sync/promotion pipeline (no
snapshot created or touched). Result, diffed GUID-for-GUID against the active VENTURE
snapshot for the same window:

| | Count |
|---|---|
| Live Tally XML export | 94, 94 unique GUIDs |
| Active VENTURE snapshot | 94, 94 unique GUIDs |
| A. Present in both | 94 |
| B. Present only in Tally | 0 |
| C. Present only in VENTURE | 0 |
...
The 94-vs-62 discrepancy is closed as **count-scope mismatch**, not a VENTURE defect —
Tally's own live, machine-readable export for the exact window VENTURE actually
queried matches VENTURE's snapshot exactly, GUID for GUID. The user's manual 62-count
in Tally's UI was almost certainly scoped to a different date range/report than
VENTURE's `2026-07-18`..`2026-08-16` window (51 of the 94 vouchers land on `2026-08-14`
...
Tally showing 97 vouchers against VENTURE's 94 — a second, related count question
surfaced mid-item-G, investigated with the same read-only discipline as §30 (no code
changes, no VENTURE sync triggered by me, no Tally writes).
...
97 total. A fresh, read-only live Tally XML query (same TDL request VENTURE's
...
live XML against the newly-promoted VENTURE snapshot (`66e7a81b...`, promoted
`2026-08-16T02:48:02Z`): **98 present in both, 0 Tally-only, 0 VENTURE-only, 0
duplicate GUIDs**, identical type breakdown. VENTURE's snapshot exactly mirrors
...
XML that VENTURE's Connector reads; subsequent ordinary voucher-entry activity in
...
This is closed as **NOT a VENTURE data-loss or filtering defect** — VENTURE correctly,
...
**97-vs-94/98-vs-98 closed as NOT A VENTURE DEFECT.** Confirmed twice, independently,
with GUID-for-GUID diffs against live Tally: VENTURE never lost, dropped, or withheld
...
behavior, not VENTURE extraction, snapshot, or API logic.
...
against loopback/localhost/both IPs, and `venture-desktop.log` correlation) traced
...
2. **Network binding** — **first failing layer.** The Connector's HTTP/HTTPS
   ...
   child's own config.
3. **Windows Firewall** — ruled out as the cause: ~11 matching Venture/Connector
...
| Installer | `VentureDesktop-0.4.13-x64-setup.exe` |
...
- A live `bonjour-service` browse of `_venture._tcp.local` from the Desktop
...
| Installer | `VentureDesktop-0.4.14-x64-setup.exe` |
...
outside `apps/venture_android` this pass.
...
| Desktop | Unchanged — `0.4.14` (this work never touched `apps/venture_desktop` or `connector`) |
...
environment condition, not a VENTURE defect. Switching the profile to Private
...
- `connector/venture_connector/src/services/discovery/mdns-advertiser.ts`:
  publishes a new, separate `securePort` TXT field, omitted entirely (never
  a placeholder) when secure transport is not running.
- `connector/venture_connector/src/bootstrap/register-services.ts`: wires the
  real `secureTransportPort`/`secureTransportEnabled` config into the
  advertiser.
- `apps/venture_android/.../core/discovery/ConnectorDiscoveryPort.kt`: adds an
  additive, nullable `securePort` field to `DiscoveredConnector`.
- `apps/venture_android/.../core/discovery/NsdConnectorDiscoveryService.kt`:
  parses the new TXT field.
- `apps/venture_android/.../core/connectorauth/domain/AuthenticatedConnectorEndpointResolver.kt`:
...
| Desktop installer | `VentureDesktop-0.4.15-x64-setup.exe` |
...
| Android artifact | `VentureAndroid-fcb936c-debug.apk` |
...
`com.jajusri.venture.debug` continuity.17/versionCode 18):
...
is an OS-level Wi-Fi auto-reconnect behavior outside VENTURE's control,
...
| Artifact | `VentureAndroid-1ae3b79-debug.apk` |
...
not a general network block and not a VENTURE code defect. Many phone-based
...
### 42.5 No new VENTURE defect this round; durable observability added
...
| Artifact | `VentureAndroid-b3a4d5c-debug.apk` |
...
Standard mode, with a real, actively-growing 15.4 MB `venture-ledger.db`. A
prior, unexplained Standard↔Private episode from 2026-08-13 was found and
preserved (not deleted) at `E:\VenturePrivate.orphaned-20260813\`. Per
...
SHA-256/size snapshot of `venture-ledger.db`/`-wal`/`-shm` captured, the user
...
`venture-ledger.db` appeared anywhere under `C:`, `LocalAppData`, or `D:` with
...
pass). The app itself showed its designed "Private VENTURE storage is not
...
same vault marker. SHA-256 hashes of `venture-ledger.db`/`-wal`/`-shm`,
...
and a live mid-write USB pull) are both explained by proven, non-VENTURE
...
|---|---|---|---|---|
| Desktop | `0.4.15` | `fcb936c320205fc87e83fecae1e4a313da2873d8` | `321c0946ff2fc263a5558bf55cc34886f7358bbf5a7eb98da674d5a6144899b5` (independently re-verified 2026-08-16) | `release/controlled-pilot/0.4.15/artifacts/VentureDesktop-0.4.15-x64-setup.exe` |
| Connector | `0.4.6` | bundled in the Desktop candidate above | — | bundled |
| Android | `0.1.1-continuity.19` (versionCode 20) | `b3a4d5c` | `33f4eef37d5d32f746e81d564ac36efa992aeb74bb6e698f7a2af0e786420c0c` (independently re-verified 2026-08-16) | `release/controlled-pilot/android/0.1.1-continuity.19-b3a4d5c/VentureAndroid-b3a4d5c-debug.apk` |
...
1. **Network:** operate on a stable, real Wi-Fi router LAN (the proven Jio
   ...
   (§45) unrelated to VENTURE. If the pilot's network genuinely must change
...
3. **USB handling:** do not remove the USB drive while VENTURE/Desktop is
...
| TD-026, TD-027 | P2 | Tally-side observational findings, not VENTURE defects, explicitly deferred |
...
`release/controlled-pilot/android/0.1.1-continuity.20-3eabf46/VentureAndroid-3eabf46-debug.apk`,
