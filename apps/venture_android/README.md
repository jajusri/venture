...
Native Android companion for VENTURE. Connects to the Venture Connector REST API.

Coexists with `apps/venture_mobile/` (Flutter). Do not remove or modify the Flutter app from this module.
...
From `apps/venture_android`. The module has two product flavors (dimension `environment`):
`prod` (application ID `com.jajusri.venture`, unchanged from before this dimension existed — this
is the flavor used for the existing production/testing device install) and `dev` (application ID
`com.jajusri.venture.dev`, label "VENTURE DEV" — installs and coexists alongside `prod` without
touching its package or data). Combined with the existing `debug`/`release` build types, task names
are flavor-qualified:

```bash
# Production/testing (matches the existing device install exactly — com.jajusri.venture.debug)
./gradlew :app:assembleProdDebug
./gradlew :app:testProdDebugUnitTest
./gradlew :app:compileProdDebugAndroidTestKotlin

# DEV (isolated — com.jajusri.venture.dev.debug — safe to install/uninstall/clear independently)
...
adb uninstall com.jajusri.venture.dev.debug   # safe: only ever targets DEV
```
