...
import com.jajusri.venture.feature.transaction.domain.port.AuthenticatedRelayRequest
import com.jajusri.venture.feature.transaction.domain.port.AuthenticatedTransportEnvelope
import com.jajusri.venture.feature.transaction.domain.port.ConfiguredRelayEndpointProvider
import com.jajusri.venture.feature.transaction.domain.port.EmptyRelayEndpointProvider
import com.jajusri.venture.feature.transaction.domain.port.EnvelopeSubmission
...
class HttpRelayClientTest {
    ...
    fun `accepted response is relay_accepted evidence and retries stay idempotent`() = runTest(dispatcher) {
        ...
        try {
            ...
            assertEquals("application/vnd.venture.order-snapshot+json", submitted.getValue("commercialContentType").jsonPrimitive.content)
    ...
    fun `mailbox fetch returns bounded relay accepted items`() = runTest(dispatcher) {
        ...
        try {
            ...
            assertEquals("application/vnd.venture.order-snapshot+json", page?.items?.single()?.commercialContentType)
