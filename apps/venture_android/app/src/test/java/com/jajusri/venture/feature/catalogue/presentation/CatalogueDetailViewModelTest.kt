...
class CatalogueDetailViewModelTest {
    ...
    fun `isManualProduct is false for a Tally-linked product`() = runTest(dispatcher) {
        val repository = FakeCatalogueRepository()
        repository.products["co-1"] = mutableListOf(
            sampleProduct(productId = "p1").copy(source = com.jajusri.venture.feature.catalogue.domain.model.CatalogueProductSource.Tally, linkedStockItemId = "guid:widget", unit = "Nos"),
        )
        val vm = viewModel(repository)
        dispatcher.scheduler.advanceUntilIdle()
    ...
    fun `loading a product also loads its existing photos`() = runTest(dispatcher) {
        val repository = FakeCatalogueRepository()
        repository.products["co-1"] = mutableListOf(sampleProduct(productId = "p1"))
        repository.addAsset("co-1", "p1", android.net.TestUri.create(), com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestamp(1L, com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestampSource.DeviceLocalProvisional))
    ...
    fun `a rejected photo surfaces a specific, honest failure message`() = runTest(dispatcher) {
        val repository = FakeCatalogueRepository()
        repository.products["co-1"] = mutableListOf(sampleProduct(productId = "p1"))
        repository.addAssetFailure = com.jajusri.venture.feature.catalogue.storage.CatalogueAssetFailureReason.FileTooLarge
        val vm = viewModel(repository)
        dispatcher.scheduler.advanceUntilIdle()
