...
import com.jajusri.venture.feature.catalogue.presentation.FakeCatalogueClock
import com.jajusri.venture.feature.catalogue.presentation.FakeCatalogueRepository
...
class CatalogueExcelImportUseCaseTest {
    ...
    fun `a Create outcome lands as a new Manual Draft with its enrichment applied`() = runTest {
        ...
        assertEquals(com.jajusri.venture.feature.catalogue.domain.model.CatalogueLifecycleState.Draft, product.lifecycleState)
    }
    ...
    fun `TD-047 -- an Update outcome for a Tally-linked product never overwrites its Tally-authoritative Unit`() = runTest {
        ...
        val existing = repository.createManualDraft(
            "co-1", "Widget",
            com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestamp(1L, com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestampSource.DeviceLocalProvisional),
        ).copy(source = com.jajusri.venture.feature.catalogue.domain.model.CatalogueProductSource.Tally, linkedStockItemId = "guid:widget", unit = "Nos")
    ...
    fun `TD (price-state lock) -- Open mode with no Price cell imports as No-Price-Supplied, never Contact-for-price`() = runTest {
        ...
        assertEquals(
            com.jajusri.venture.feature.catalogue.domain.model.CataloguePriceState.NoPriceSupplied,
            com.jajusri.venture.feature.catalogue.domain.model.resolveCataloguePriceState(
    ...
    fun `an Update outcome applies enrichment to the existing product without changing its identity`() = runTest {
        val repository = FakeCatalogueRepository()
        val existing = repository.createManualDraft("co-1", "Widget", com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestamp(1L, com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestampSource.DeviceLocalProvisional))
        val useCase = CatalogueExcelImportUseCase(repository, FakeCatalogueClock())
        val rows = listOf(
            row(
                1,
    ...
    fun `a blank custom column on re-import clears a previously-set value`() = runTest {
        val repository = FakeCatalogueRepository()
        val ts = com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestamp(1L, com.jajusri.venture.feature.catalogue.domain.model.CatalogueTimestampSource.DeviceLocalProvisional)
        ...
        repository.updateEnrichment("co-1", existing.productId, com.jajusri.venture.feature.catalogue.domain.repository.CatalogueEnrichmentUpdate(sku = "SKU-1"), ts)
