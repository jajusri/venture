...
class PartyRelatedDaoTest {
    ...
    fun fieldProvenance_everyRequiredStateRoundTrips() = runBlocking {
        val states = listOf("confirmed_from_tally", "venture_only_pending", "export_ready", "exported", "conflict", "empty_unknown")
    ...
    fun fieldProvenance_upsertReplacesOnSameCompositeKey() = runBlocking {
        fieldProvenanceDao.upsert(provenance("co-a", "party-1", "primaryPhone", "venture_only_pending", ventureValue = "9111111111"))
    ...
    fun contactPersons_multiplePeopleWithPrimaryOrderedFirst() = runBlocking {
        ...
        contactPersonDao.upsert(contact("co-a", "cp-3", "party-1", "Accounts", isPrimary = false, provenance = "venture_only"))

        val all = contactPersonDao.findAllForParty("co-a", "party-1")
        assertEquals(3, all.size)
        assertTrue(all.first().isPrimary)
        assertEquals("Owner", all.first().name)
        assertTrue(all.any { it.provenance == "venture_only" })
    }
