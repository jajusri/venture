...
import com.jajusri.venture.feature.voucher.data.local.VoucherDao
...
object DatabaseModule {
    ...
    @Provides
    fun provideEstimatePoDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.EstimatePoDao = db.estimatePoDao()

    @Provides
    fun provideEstimatePoLineItemDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.EstimatePoLineItemDao = db.estimatePoLineItemDao()

    @Provides
    fun provideSellerInboxEntryDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.SellerInboxEntryDao = db.sellerInboxEntryDao()

    @Provides
    fun provideCommercialTransactionDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.CommercialTransactionDao = db.commercialTransactionDao()

    @Provides
    fun provideTermsAcknowledgmentDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.TermsAcknowledgmentDao = db.termsAcknowledgmentDao()

    @Provides
    fun providePaymentEventDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.PaymentEventDao = db.paymentEventDao()

    @Provides
    fun provideLedgerIntentDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.LedgerIntentDao = db.ledgerIntentDao()

    @Provides
    fun provideCatalogueAccessGrantDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.CatalogueAccessGrantDao = db.catalogueAccessGrantDao()

    @Provides
    fun provideCanonicalOrderDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.CanonicalOrderDao = db.canonicalOrderDao()

    @Provides
    fun provideOrderOutboxDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.OrderOutboxDao = db.orderOutboxDao()

    @Provides
    fun provideStructuredRecipientInboxDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.StructuredRecipientInboxDao =
        db.structuredRecipientInboxDao()

    @Provides
    fun provideRecipientInboxCursorDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.RecipientInboxCursorDao =
        db.recipientInboxCursorDao()

    @Provides
    fun provideOrderCommercialEventDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.OrderCommercialEventDao =
        db.orderCommercialEventDao()

    @Provides
    fun provideOrderVersionArchiveDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.OrderVersionArchiveDao =
        db.orderVersionArchiveDao()

    @Provides
    fun provideAuthenticatedCounterpartyBindingDao(db: AppDatabase): com.jajusri.venture.feature.transaction.data.local.AuthenticatedCounterpartyBindingDao =
     ...
     * [com.jajusri.venture.feature.masterdata.ledger.data.local.LedgerMovementDao]'s doc comment)
     ...
     * themselves are seeded separately by [com.jajusri.venture.feature.party.domain.usecase.ReconcilePartiesFromLedgersUseCase]
        ...
        override fun migrate(db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                "CREATE TABLE IF NOT EXISTS `party_field_provenance` (" +
                    ...
                    "`state` TEXT NOT NULL, `tallyValue` TEXT, `ventureValue` TEXT, `lastConfirmedAt` INTEGER, " +
    ...
    /**
     * Additive-only: adds the single MVP-1.1-C `party_notes` table (VENTURE-only Party notes,
    ...
    /**
     * Additive-only (MVP-1.3-A): adds the single `business_profile` table — one VENTURE-owned row
    ...
    /**
     * Transaction Mode foundation (docs/architecture/VENTURE-TRANSACTION-MODE-ARCHITECTURE.md §14):
