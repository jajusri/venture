...
class PartyDetailViewModel @Inject constructor(
    ...
    private val updateVentureOnlyField: UpdateVentureOnlyFieldUseCase,
    ...
    private fun load() {
        viewModelScope.launch {
            ...
            val fieldRows = PARTY_DETAIL_FIELD_LABELS.map { (fieldName, label) ->
                ...
                val state = provenance[fieldName]?.state ?: if (effectiveValue.isNullOrBlank()) FieldProvenanceState.EmptyUnknown else FieldProvenanceState.VentureOnlyPending
                PartyFieldRowUi(
                    fieldName = fieldName,
                    label = label,
    ...
    private fun saveEditedField() {
        ...
        val companyId = _uiState.value.companyId ?: return
        viewModelScope.launch {
            updateVentureOnlyField(companyId, partyId, dialog.fieldName, dialog.currentValue.trim().ifEmpty { null })
     ...
     * changes [com.jajusri.venture.feature.party.domain.model.PartyNote.completedAt], preserving its
