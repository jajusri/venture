...
internal object LedgerStatementPdfRenderer {
    ...
    fun render(statement: LedgerStatement, companyName: String?, generatedAtEpochMillis: Long, output: File) {
        ...
        try {
            (1..totalPages).forEach { pageNumber ->
                ...
                if (pageNumber == 1) {
                    canvas.drawText("VENTURE LEDGER STATEMENT", PAGE_WIDTH / 2f, 24f, heading)
    ...
    private fun drawFooter(
        ...
        canvas.drawText(
            "Generated ${formatGeneratedAt(generatedAtEpochMillis)} from synchronized Tally data in VENTURE.",
            MARGIN,
            FOOTER_TOP + 12f,
            footer,
        )
    ...
    private fun com.jajusri.venture.feature.masterdata.ledger.domain.model.LedgerStatementAmount?.toLabel(): String? {
