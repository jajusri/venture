...
import com.jajusri.venture.feature.party.data.local.PartyTagAssignmentRow
...
import com.jajusri.venture.feature.party.data.local.asColumn
import com.jajusri.venture.feature.party.data.local.toDomain
import com.jajusri.venture.feature.party.data.local.toFieldProvenanceState
...
class PartyRepositoryImpl @Inject constructor(
     ...
     * as a Party -- see [com.jajusri.venture.feature.party.domain.model.LedgerPartyEligibilityPolicy]),
    ...
    override suspend fun updateVentureOnlyField(
        ...
        val newState = if (value.isNullOrBlank()) FieldProvenanceState.EmptyUnknown else FieldProvenanceState.VentureOnlyPending
    ...
    override suspend fun confirmFieldFromTally(
        ...
        val now = timeProvider.nowEpochMillis()
        val existing = fieldProvenanceDao.findField(companyId, partyId, fieldName)
        val pendingVentureValue = existing?.ventureValue
        val newState = when {
            tallyValue.isNullOrBlank() -> FieldProvenanceState.EmptyUnknown
            pendingVentureValue != null && pendingVentureValue != tallyValue -> FieldProvenanceState.Conflict
     ...
     * phone, email/state/GSTIN legitimately vary in case/formatting between Tally and VENTURE
     ...
     * Indian-mobile validation may seed the VENTURE phone field, but only non-destructively — an
    ...
    override suspend fun createProspect(companyId: String, draft: ProspectDraft): Party = withContext(dispatchers.io) {
        ...
        // No Tally source link is created — a Prospect is VENTURE-native by definition (spec
        ...
        listOfNotNull(
            ...
            fieldProvenanceDao.upsert(
                baseProvenance(null, companyId, partyId, fieldName).copy(
                    state = FieldProvenanceState.VentureOnlyPending.asColumn(),
                    ventureValue = value,
                    updatedAt = now,
                ),
            )
    ...
    // ---- Transaction Mode integration point (docs/architecture/VENTURE-TRANSACTION-MODE-ARCHITECTURE.md §6) ----
    ...
    override suspend fun upsertContactPerson(
        ...
        val entity = PartyContactPersonEntity(
            ...
            email = email,
            isPrimary = isPrimary,
            provenance = FieldProvenanceState.VentureOnlyPending.asColumn(),
    ...
    override suspend fun reconcileExportedFieldFromTally(
        ...
        val canonicalVenture = canonicalizeForComparison(fieldName, existing?.ventureValue)
        val newState = if (canonicalVenture != null && canonicalVenture != canonicalTally) {
