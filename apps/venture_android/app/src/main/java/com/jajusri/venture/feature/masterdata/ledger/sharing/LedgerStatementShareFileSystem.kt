...
/**
 * Path-containment boundary for the ledger-statement PDF cache directory — same shape and same
 * security properties as [com.jajusri.venture.feature.voucher.sharing.InvoiceShareCacheBoundary],
 * deliberately kept as its own instance (own directory, own mutable cleanup state) rather than
 * reusing that singleton: mixing two unrelated file domains into one boundary/cleanup instance
 * would make each domain's cache pressure and lifecycle affect the other's. Reuses
 * [InvoiceShareFileOperations] directly since that wrapper is already directory-agnostic.
 */
@Singleton
class LedgerStatementShareCacheBoundary internal constructor(
    cacheRoot: File,
    ...
    private val rawDirectory = File(cacheRoot, LedgerStatementShareCachePolicy.CACHE_DIRECTORY)
    ...
    private val boundaryIsSafe = canonicalCacheRoot != null &&
        ...
        File(normalizedRawDirectory).name == LedgerStatementShareCachePolicy.CACHE_DIRECTORY &&
        File(expectedDirectoryPath).name == LedgerStatementShareCachePolicy.CACHE_DIRECTORY
...
/**
 * Bounded age/count-based cache eviction for the ledger-statement PDF cache directory — same
 * policy shape as [com.jajusri.venture.feature.voucher.sharing.InvoiceShareCachePolicy].
 */
@Singleton
class LedgerStatementShareCachePolicy @Inject constructor(
    private val boundary: LedgerStatementShareCacheBoundary,
    ...
    companion object {
        internal const val CACHE_DIRECTORY = "ledger-statement-share"
