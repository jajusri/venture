package com.jajusri.venture.feature.party.domain.repository
 ...
 * either seeded from already-synced local Ledger data or edited directly in VENTURE.
 */
interface PartyRepository {
     ...
     * already used as [com.jajusri.venture.feature.masterdata.ledger.domain.model.Ledger.id]). */
    ...
    /** A VENTURE-side edit to a Tally-compatible field. Always lands as [FieldProvenanceState.VentureOnlyPending]
     * — never becomes confirmed merely because the user entered it (architecture §5/§6.2). */
    suspend fun updateVentureOnlyField(companyId: String, partyId: String, fieldName: String, value: String?): FieldProvenanceState

    /** Models "a later Tally sync returned this exact field" — the only path that can promote a
     * field to [FieldProvenanceState.ConfirmedFromTally]. A non-null pending VENTURE value that
    ...
    /** Creates a VENTURE-native Prospect: a Party with no Tally source link at all. Works fully
     ...
     * the pre-1.2-A one-tap "just write a note" behavior (see [com.jajusri.venture.feature.party.domain.usecase.AddNoteUseCase]
    ...
    /** One review row per [com.jajusri.venture.feature.party.domain.model.TallyExportFieldMapping]-
