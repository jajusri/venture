...
async function refreshDriveList(driveListEl: HTMLElement, noDrivesEl: HTMLElement): Promise<void> {
  const volumes = await window.ventureDesktop.listRemovableVolumes();
...
/**
 * Shows the first-run "Where should VENTURE keep your private business data?" setup screen and
    ...
    const onContinue = (): void => {
      ...
      void (async () => {
        ...
        try {
          ...
          const result = await window.ventureDesktop.chooseStorageMode({ ...choice, confirmSwitch });
...
/** Shows the "Private VENTURE storage is not connected" screen with Retry / Locate / Exit. */
async function runUnavailableFlow(overlay: HTMLElement, reason: 'missing' | 'mismatched'): Promise<void> {
  ...
  detailEl.textContent = reason === 'mismatched'
    ? 'The drive at the expected letter now holds different removable media. Connect the correct VENTURE storage device.'
    : 'Connect the removable storage device VENTURE was configured to use, then retry.';
  ...
  await new Promise<void>((resolve) => {
    const onRetry = (): void => {
      ...
      void (async () => {
        try {
          const state = await window.ventureDesktop.retryStorageConnection();
          if (state.kind === 'ready') {
            cleanup();
            hide(overlay);
            resolve();
...
async function waitForStorageGateResolution(initial: StorageGateState): Promise<StorageGateState> {
  ...
  while (state.kind === 'resolving' && Date.now() < deadline) {
    ...
    state = await window.ventureDesktop.getStorageStatus();
  }
  return state;
}
...
export async function renderStorageGate(): Promise<boolean> {
  let state: StorageGateState = await window.ventureDesktop.getStorageStatus();
