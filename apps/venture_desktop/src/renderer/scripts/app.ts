...
export async function refreshDashboardDataFreshness(): Promise<void> {
  ...
  const bridge = window.ventureDesktop;
...
export async function refreshDiagnostics(): Promise<void> {
  ...
  try {
    const snapshot = await window.ventureDesktop.refreshDiagnostics();
...
export async function handleCompanySelection(companyId: string): Promise<void> {
  ...
  renderConnectionDisplay();
  try {
    const outcome = await window.ventureDesktop.selectCompany(companyId);
    if (!outcome.ok) {
...
export async function handleClearCompany(): Promise<void> {
  ...
  try {
    await window.ventureDesktop.clearCompany();
    resetPerCompanyModuleState();
    await refreshUi({ showLoading: false });
    await loadCompanies();
...
export function bindLifecycleActions(): void {
  document.getElementById('btn-start-connector')?.addEventListener('click', () => {
    void runLifecycleAction('Starting connector…', () => window.ventureDesktop.startConnector());
  });
  document.getElementById('btn-stop-connector')?.addEventListener('click', () => {
    void runLifecycleAction('Stopping connector…', () => window.ventureDesktop.stopConnector());
  });
  document.getElementById('btn-restart-connector')?.addEventListener('click', () => {
    void runLifecycleAction('Restarting connector…', () => window.ventureDesktop.restartConnector());
  });
}
...
async function applyDetectedTrustedLanHost(hostInput: HTMLInputElement): Promise<void> {
  try {
    const status = await window.ventureDesktop.getMobileAccessStatus();
...
async function saveSettings(): Promise<void> {
  ...
  try {
    ...
    const validation = await window.ventureDesktop.validateSettings(payload);
    if (!validation.ok) {
      renderValidationErrors(validation.errors);
      renderSettingsStatus('Fix validation errors before saving.', true);
      return;
    }
    const result = await window.ventureDesktop.saveSettings(payload);
...
async function restoreSettings(): Promise<void> {
  setLoading({ settings: true });
  try {
    const result = await window.ventureDesktop.restoreDefaultSettings();
    if (result.settings) {
      settingsDirty = false;
      renderSettingsForm(result.settings);
    }
...
export function bindDiagnosticsActions(): void {
  ...
  document.getElementById('btn-open-logs-folder')?.addEventListener('click', () => {
    void window.ventureDesktop.openLogsFolder();
  });
  ...
  document.getElementById('btn-reload-renderer')?.addEventListener('click', () => {
    void window.ventureDesktop.reloadRenderer();
  });
}

async function copyDiagnostics(): Promise<void> {
  try {
    const summary = await window.ventureDesktop.copyDiagnosticsSummary();
...
async function exportDiagnostics(): Promise<void> {
  setLoading({ diagnostics: true });
  try {
    const result = await window.ventureDesktop.exportDiagnosticsBundle();
...
async function runHealthCheckAction(): Promise<void> {
  setLoading({ diagnostics: true });
  try {
    const result = await window.ventureDesktop.runHealthCheck();
    setText('diagnostics-status-message', result.message);
...
async function clearLogsAction(): Promise<void> {
  const result = await window.ventureDesktop.clearNonessentialLogs();
  setText('diagnostics-status-message', result.message);
  await refreshUi({ showLoading: false });
}
...
export async function loadLedgers(options: { readonly showLoading?: boolean } = {}): Promise<void> {
  ...
  try {
    const state = await window.ventureDesktop.getLedgers({
      query: ledgerQuery,
      page: ledgerPage,
...
async function handleLedgerSync(): Promise<void> {
  ...
  startLedgerProgressPolling();
  try {
    const result = await window.ventureDesktop.syncLedgers(false);
...
async function handleCancelLedgerSync(): Promise<void> {
  try {
    const result = await window.ventureDesktop.cancelLedgerSync();
...
async function handleClearLedgerCache(): Promise<void> {
  ...
  try {
    const result = await window.ventureDesktop.clearLedgerCache();
...
export async function loadStockItems(options: { readonly showLoading?: boolean } = {}): Promise<void> {
  ...
  try {
    const state = await window.ventureDesktop.getStockItems({
...
async function pollPairingStatus(): Promise<void> {
  ...
  try {
    pairingSession = await window.ventureDesktop.getPairingStatus();
...
async function loadTrustedPairingDevices(): Promise<void> {
  try {
    trustedPairingDevices = await window.ventureDesktop.listTrustedPairingDevices();
  } catch {
...
export async function loadPairingPanel(): Promise<void> {
  try {
    pairingCapability = await window.ventureDesktop.getSecurePairingCapability();
  } catch {
    pairingCapability = {
      state: 'unavailable',
      connectorName: null,
...
async function handleEnableSecurePairing(): Promise<void> {
  ...
  try {
    const result = await window.ventureDesktop.enableSecurePairing();
...
async function handleStartPairing(): Promise<void> {
  ...
  pairingActionInFlight = true;
  try {
    pairingSession = await window.ventureDesktop.startPairing();
    renderPairingSession();
...
async function handleCancelPairing(): Promise<void> {
  try {
    pairingSession = await window.ventureDesktop.cancelPairing();
    renderPairingSession();
  } catch {
...
async function handleRevokeTrustedDevice(credentialId: string): Promise<void> {
  ...
  try {
    const result = await window.ventureDesktop.revokeTrustedPairingDevice(credentialId);
    setBanner(result.message, result.ok ? 'information' : 'warning');
    await loadTrustedPairingDevices();
    await loadPairingPanel();
  } catch {
...
async function handleStockItemSync(): Promise<void> {
  ...
  startStockItemProgressPolling();
  try {
    const result = await window.ventureDesktop.syncStockItems(false);
...
async function handleCancelStockItemSync(): Promise<void> {
  try {
    const result = await window.ventureDesktop.cancelStockItemSync();
...
async function handleClearStockItemCache(): Promise<void> {
  ...
  try {
    const result = await window.ventureDesktop.clearStockItemCache();
    setBanner(result.message, result.ok ? 'information' : 'warning');
...
async function ensureStorageGateReady(): Promise<void> {
  ...
  try {
    const status = await window.ventureDesktop.getStorageStatus();
...
export async function startDesktopShell(): Promise<void> {
  ...
  window.ventureDesktop.onStatusUpdated(() => {
    ...
    void (async () => {
      ...
      // used at startup reuses the purpose-built "Private VENTURE storage is not connected" screen
...
if (typeof window !== 'undefined' && window.ventureDesktop) {
