/**
 * The Connector-side default (connector/venture_connector/src/config/defaults.ts,
 ...
 * (VENTURE_SECURE_PAIRING_ENABLED, VENTURE_SECURE_TRANSPORT_ENABLED, VENTURE_SECURE_TRANSPORT_PORT,
 * VENTURE_DESKTOP_CONTROL_TOKEN — see connector/venture_connector/src/config/index.ts). Each must
...
export function buildSecurePairingChildEnvOverrides(
  ...
  return {
    ...
    VENTURE_SECURE_TRANSPORT_PORT: String(options.securePort ?? DEFAULT_SECURE_TRANSPORT_PORT),
    VENTURE_DESKTOP_CONTROL_TOKEN: options.controlToken,
  };
}
