...
export class HttpHealthChecker implements HealthChecker {
  ...
  constructor(
    ...
    ownership?: {
       ...
       * known before any Connector process exists and always injected as VENTURE_CONNECTOR_ID
    ...
    try {
      ...
      const process = this.processSpawner.spawn({
        ...
        env: {
          ...
          VENTURE_CONNECTOR_HOST: this.config.connectorHost,
          VENTURE_CONNECTOR_PORT: String(this.config.connectorPort),
          ...(this.config.startupCorrelationId
            ? { VENTURE_STARTUP_CORRELATION_ID: this.config.startupCorrelationId }
            : {}),
        },
      });
