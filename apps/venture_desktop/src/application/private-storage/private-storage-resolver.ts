...
import {
  VENTURE_PRIVATE_ROOT_FOLDER,
  VENTURE_VAULT_MARKER_FILE,
...
function ventureRootOf(driveLetter: string): string {
  return path.join(driveLetter, VENTURE_PRIVATE_ROOT_FOLDER);
}

function markerPathOf(driveLetter: string): string {
  return path.join(ventureRootOf(driveLetter), VENTURE_VAULT_MARKER_FILE);
}

/** `<drive>\VenturePrivate\<vaultId>\connector-data` — mirrors the AppData layout's connectorDataDir shape. */
export function privateConnectorDataDir(driveLetter: string, vaultId: string): string {
  return path.join(ventureRootOf(driveLetter), vaultId, 'connector-data');
}

/**
 * TD-034: a marker file that exists but fails to parse (e.g. a BOM-prefixed write from a tool
 * other than VENTURE's own `JSON.stringify`, or partial/corrupt content) must never be silently
...
export class UnreadablePrivateVaultMarkerError extends Error {
  constructor(driveLetter: string, cause: unknown) {
    super(
      `A VENTURE private-storage marker already exists on ${driveLetter} but could not be read` +
        `${cause instanceof Error ? ` (${cause.message})` : ''}. To avoid orphaning whatever data ` +
        'it points to, VENTURE will not create a new vault here. Choose a different drive, or ' +
...
/**
 * Reads whatever VENTURE vault marker already exists on [driveLetter], if any — used when the
 ...
 * connector-data directory, but never touches any pre-existing VenturePrivate folder on that
 * drive — if one already exists (e.g. the user picked a drive that already has a VENTURE vault
...
export function createPrivateVault(
  ...
  if (existing) {
    throw new Error(
      `A VENTURE private vault already exists on ${driveLetter} (vaultId ${existing.vaultId}). Choose a different drive or adopt the existing vault.`,
    );
  }
  const marker: PrivateStorageVaultMarker = { schemaVersion: 1, vaultId, createdAt: new Date().toISOString() };
  fsImpl.mkdirSync(ventureRootOf(driveLetter), { recursive: true });
...
export async function resolvePrivateVault(
  ...
  for (const volume of volumes) {
    ...
    if (marker && marker.vaultId !== expectedVaultId && volume.driveLetter === locator.lastKnownDriveLetter) {
      // The last-known letter now holds a *different* VENTURE vault (lookalike media) — worth
...
export function hasExistingStandardModeDatabase(
  ...
  return fsImpl.existsSync(path.join(connectorDataDir, 'venture-ledger.db'));
}
