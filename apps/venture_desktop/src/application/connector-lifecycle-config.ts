...
export interface ConnectorLifecycleResolutionContext {
   ...
   * to the spawned Connector as VENTURE_TRANSPORT_IDENTITY_DIR so it never falls back to its own
   ...
   * Connector child process as VENTURE_CONNECTOR_ID so it never has to generate its own.
   */
  readonly connectorId?: string;
  /**
   * Private Removable Storage mode only. When both are set, passed through as
   * VENTURE_PRIVATE_STORAGE_VAULT_ID / VENTURE_PRIVATE_STORAGE_MARKER_PATH so the Connector's own
   ...
   * the spawned Connector as VENTURE_TALLY_HOST/VENTURE_TALLY_PORT (TD-004) so it never silently
...
export function resolveConnectorLifecycleConfig(
  ...
  const connectorBaseUrlInput = overrides.connectorBaseUrl ?? process.env.VENTURE_CONNECTOR_URL ?? 'http://127.0.0.1:8080';
  const parsedHostPort = parseConnectorHostPort(connectorBaseUrlInput);
  const resolvedBindMode = overrides.connectorBindMode
    ?? (process.env.VENTURE_CONNECTOR_BIND_MODE?.trim().toLowerCase() === 'trusted-lan'
      ? 'trusted-lan'
      : process.env.VENTURE_CONNECTOR_BIND_MODE?.trim().toLowerCase() === 'local-only'
        ? 'local-only'
        : undefined)
  ...
  const lanModeAcknowledged =
    ...
    || process.env.VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED?.trim().toLowerCase() === 'true';
  ...
  let connectorExecutable = overrides.connectorExecutable ?? process.env.VENTURE_CONNECTOR_EXECUTABLE ?? '';
  ...
  const connectorArgs = overrides.connectorArgs
    ?? (process.env.VENTURE_CONNECTOR_ARGS
      ? process.env.VENTURE_CONNECTOR_ARGS.split(' ')
      : [defaultScript]);
  const connectorCwd = overrides.connectorCwd
    ?? process.env.VENTURE_CONNECTOR_CWD
    ?? (isPackaged
  ...
  const childEnvOverrides: Record<string, string> = {
    VENTURE_CONNECTOR_PORT: String(connectorPort),
    VENTURE_CONNECTOR_HOST: connectorHost,
    VENTURE_CONNECTOR_BIND_MODE: resolvedBindMode,
    VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED: String(lanModeAcknowledged),
  };
  const startupCorrelationId = overrides.startupCorrelationId
    ?? process.env.VENTURE_STARTUP_CORRELATION_ID?.trim()
    ?? null;
  if (startupCorrelationId) {
    childEnvOverrides.VENTURE_STARTUP_CORRELATION_ID = startupCorrelationId;
  }
  if (context.connectorDatabaseDir) {
    childEnvOverrides.VENTURE_DATABASE_PATH = context.connectorDatabaseDir;
  }
  if (context.connectorTallyAuditPath) {
    childEnvOverrides.VENTURE_TALLY_REQUEST_AUDIT_PATH = context.connectorTallyAuditPath;
  }
  if (context.connectorVoucherSyncFailureAuditPath) {
    childEnvOverrides.VENTURE_VOUCHER_SYNC_FAILURE_AUDIT_PATH = context.connectorVoucherSyncFailureAuditPath;
  }
  if (context.connectorTransportIdentityDir) {
    childEnvOverrides.VENTURE_TRANSPORT_IDENTITY_DIR = context.connectorTransportIdentityDir;
  }
  if (context.connectorId) {
    childEnvOverrides.VENTURE_CONNECTOR_ID = context.connectorId;
  }
  if (context.privateStorageExpectedVaultId) {
    childEnvOverrides.VENTURE_PRIVATE_STORAGE_VAULT_ID = context.privateStorageExpectedVaultId;
  }
  if (context.privateStorageMarkerPath) {
    childEnvOverrides.VENTURE_PRIVATE_STORAGE_MARKER_PATH = context.privateStorageMarkerPath;
  }
  if (context.tallyHost) {
    childEnvOverrides.VENTURE_TALLY_HOST = context.tallyHost;
  }
  if (context.tallyPort !== undefined) {
    childEnvOverrides.VENTURE_TALLY_PORT = String(context.tallyPort);
  }
  ...
  return {
    ...
    autoStart: overrides.autoStart ?? process.env.VENTURE_CONNECTOR_AUTO_START !== 'false',
