...
export function applyEnvironmentOverrides(
  ...
  const envUrl = process.env.VENTURE_CONNECTOR_URL;
  ...
  const envHost = process.env.VENTURE_CONNECTOR_HOST;
  if (envHost) {
    config = {
      ...config,
  ...
  const envPort = readEnvNumber('VENTURE_CONNECTOR_PORT');
  if (envPort !== undefined && urlDerivedPort === undefined) {
    config = { ...config, connectorPort: envPort };
    sources.connectorPort = 'environment';
  }

  const bindMode = readEnvBindMode('VENTURE_CONNECTOR_BIND_MODE');
  if (bindMode) {
    config = {
      ...config,
