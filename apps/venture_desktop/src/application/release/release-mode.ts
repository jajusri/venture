/**
 * Authoritative release-mode contract for Venture desktop.
 *
 * Precedence (first match wins):
 * 1. VENTURE_RELEASE_MODE environment variable
...
export function resolveReleaseMode(input: ResolveReleaseModeInput = {}): ReleaseMode {
  const raw = input.envValue ?? process.env.VENTURE_RELEASE_MODE ?? input.buildInfoMode;
