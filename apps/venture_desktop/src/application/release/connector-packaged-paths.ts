...
export function resolvePackagedConnectorPaths(
  ...
  const devScript = path.join(repoRoot, 'connector', 'venture_connector', 'dist', 'main.js');
  const fromDistMain = path.resolve(__dirname, '../../../../connector/venture_connector/dist/main.js');
...
export const CONNECTOR_CHILD_ENV_ALLOWLIST = [
  ...
  'APPDATA',
  'LOCALAPPDATA',
  'VENTURE_RELEASE_MODE',
  'VENTURE_DATABASE_PATH',
  'VENTURE_TALLY_REQUEST_AUDIT_PATH',
  'VENTURE_VOUCHER_SYNC_FAILURE_AUDIT_PATH',
  'VENTURE_TRANSPORT_IDENTITY_DIR',
  'VENTURE_PRIVATE_STORAGE_VAULT_ID',
  'VENTURE_PRIVATE_STORAGE_MARKER_PATH',
  'VENTURE_CONNECTOR_PORT',
  'VENTURE_CONNECTOR_HOST',
  'VENTURE_CONNECTOR_BIND_MODE',
  'VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED',
  'VENTURE_CONNECTOR_ID',
  'VENTURE_CONNECTOR_NAME',
  'VENTURE_TALLY_HOST',
  'VENTURE_TALLY_PORT',
  'VENTURE_STARTUP_CORRELATION_ID',
  'VENTURE_SECURE_PAIRING_ENABLED',
  'VENTURE_SECURE_TRANSPORT_ENABLED',
  'VENTURE_SECURE_TRANSPORT_PORT',
  'VENTURE_DESKTOP_CONTROL_TOKEN',
  'VENTURE_LOG_LEVEL',
  'NODE_ENV',
  ...
  if (!discovered) {
    throw new Error(
      'No compatible development Node runtime was found on PATH. Electron cannot host the Connector directly. ' +
        'Install Node.js 22+ so "node" resolves on PATH, or set VENTURE_CONNECTOR_EXECUTABLE to a compatible ' +
        'node executable.',
    );
  }
  ...
  if (shouldValidateSqlite && !sqliteChecker(discovered)) {
    throw new Error(
      `Development Node runtime at "${discovered}" does not support node:sqlite, which the Connector requires. ` +
        'Install Node.js 22+ and ensure it resolves first on PATH, or set VENTURE_CONNECTOR_EXECUTABLE to a ' +
