...
if (!process.env.VENTURE_STARTUP_CORRELATION_ID?.trim()) {
  process.env.VENTURE_STARTUP_CORRELATION_ID = crypto.randomUUID();
}
const startupCorrelationId = process.env.VENTURE_STARTUP_CORRELATION_ID.trim();

const skipSingleInstance = process.env.VITEST === 'true' || process.env.VENTURE_SKIP_SINGLE_INSTANCE === 'true';

function startupLog(stage: string, detail?: string): void {
  const suffix = detail ? ` — ${detail}` : '';
  console.error(`[venture-desktop:startup] ${stage}${suffix}`);
}
...
const settingsService = new SettingsService({
  ...
  logService,
  isDevelopment,
  connectorExecutable: process.env.VENTURE_CONNECTOR_EXECUTABLE ?? process.execPath,
  lifecycleContext: {
    isPackaged: app.isPackaged,
...
function startPrivateStorageWatchdog(driveLetter: string, vaultId: string): void {
  ...
  privateStorageWatchdogTimer = setInterval(() => {
    ...
    logService.appendStructured({
      level: 'warning',
      message: 'Private VENTURE storage disconnected while running.',
    ...
    if (choice.mode === 'standard') {
      ...
      if (isSwitch && !choice.confirmSwitch) {
        return {
          ...
          message:
            'This will stop using your current private removable storage. VENTURE will not delete ' +
    ...
    if (isSwitch && !choice.confirmSwitch) {
      return {
        ...
        message: existingLocator?.mode === 'standard'
          ? 'This will stop using VENTURE\'s built-in storage and switch to this removable drive. ' +
