...
async function main(): Promise<void> {
  ...
  checks.push(runCheck('connector-network-policy', () => {
    const defaults = getEnvironmentDefaults(true);
    const lanAck = process.env.VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED === 'true';
    if (!isConnectorLanModePolicySatisfied(defaults.connectorHost, lanAck)) {
      throw new Error(
        'Non-loopback connector host configured without VENTURE_CONNECTOR_LAN_MODE_ACKNOWLEDGED=true.',
      );
    }
  ...
  checks.push(await runAsyncCheck('diagnostics-export', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-diag-'));
  ...
  checks.push(runCheck('log-writer', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-log-'));
