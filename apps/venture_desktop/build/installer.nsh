...
Function td018Trace
  ...
  Push $1
  ClearErrors
  FileOpen $1 "$TEMP\Venture-TD018-installer.log" a
...
Function td018ConsiderLegacyRoot
  ...
  td018_consider_partial:
    ...
    StrCpy $td018MigrationError "A legacy Venture transport identity is incomplete. Installation stopped before replacing the existing application."
  ...
  td018_consider_conflict:
    ...
    StrCpy $td018MigrationError "Different legacy Venture transport identities were found. Installation stopped to preserve secure pairing."
    Goto td018_consider_done
...
!macro preInit
  !ifndef BUILD_UNINSTALLER
    ...
    Delete "$TEMP\Venture-TD018-installer.log"
...
!macro customInit
  ...
  StrCpy $td018PersistentDir "$td018UserAppData\@venture\desktop\connector-transport-identity"
  ...
  td018_destination_partial:
    ...
    Abort "The persistent Venture transport identity is incomplete. Installation stopped without replacing the existing application."
  ...
  td018_find_sources:
    ...
    StrCpy $td018StageDir "$td018UserAppData\@venture\desktop\connector-transport-identity.migrating-$0"
  ...
  td018_stage_collision:
    ...
    Abort "A Venture identity migration staging path already exists. Installation stopped without replacing the existing application."

  td018_destination_changed:
    !insertmacro td018Trace "abort=destination_changed"
    RMDir /r "$td018StageDir"
    Abort "The persistent Venture transport identity changed during installation. Installation stopped without replacing the existing application."

  td018_copy_failed:
    !insertmacro td018Trace "abort=copy_or_verify_failed"
    RMDir /r "$td018StageDir"
    Abort "The legacy Venture transport identity could not be copied and verified. Installation stopped without replacing the existing application."

  td018_publish_failed:
    !insertmacro td018Trace "abort=publish_failed"
    RMDir /r "$td018StageDir"
    Abort "The preserved Venture transport identity could not be published atomically. Installation stopped without replacing the existing application."
...
!macro customInstall
  ...
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall delete rule name="Venture Connector HTTP"'
  Pop $0
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall delete rule name="Venture Connector HTTPS"'
  Pop $0
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall delete rule name="Venture Connector mDNS"'
  Pop $0
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall add rule name="Venture Connector HTTP" dir=in action=allow program="$INSTDIR\resources\node\node.exe" enable=yes profile=private,public protocol=TCP localport=8080 remoteip=LocalSubnet'
  Pop $0
  StrCmp $0 "0" +2
    Abort "Unable to create the Venture Connector HTTP firewall rule."
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall add rule name="Venture Connector HTTPS" dir=in action=allow program="$INSTDIR\resources\node\node.exe" enable=yes profile=private,public protocol=TCP localport=8443 remoteip=LocalSubnet'
  Pop $0
  StrCmp $0 "0" +2
    Abort "Unable to create the Venture Connector HTTPS firewall rule."
  nsExec::ExecToLog '"$SYSDIR\netsh.exe" advfirewall firewall add rule name="Venture Connector mDNS" dir=in action=allow program="$INSTDIR\resources\node\node.exe" enable=yes profile=private,public protocol=UDP localport=5353 remoteip=LocalSubnet'
  Pop $0
  StrCmp $0 "0" +2
    Abort "Unable to create the Venture Connector mDNS firewall rule."
!macroend
