...
describe('DiagnosticExportRetentionService', () => {
  it('1. treats a missing export directory as a successful no-op', () => {
    ...
    const result = cleanupAt(service, path.join(os.tmpdir(), 'venture-missing-exports'), 14, Date.now());
  ...
  it('2. treats an empty directory as a successful no-op', () => {
    const exportRoot = mkTempDir('venture-empty-exports-');
  ...
  it('3. never deletes unrelated files', () => {
    const exportRoot = mkTempDir('venture-unrelated-exports-');
  ...
  it('4. never deletes backup files', () => {
    const exportRoot = mkTempDir('venture-backup-exports-');
  ...
  it('5. never deletes temporary or non-owned files', () => {
    const exportRoot = mkTempDir('venture-temp-exports-');
    fs.writeFileSync(path.join(exportRoot, 'partial-export.tmp'), 'temp', 'utf8');
    fs.mkdirSync(path.join(exportRoot, 'venture-diagnostics-not-a-valid-timestamp'), { recursive: true });
    const now = Date.parse('2026-07-25T12:00:00.000Z');
    ...
    expect(fs.existsSync(path.join(exportRoot, 'venture-diagnostics-not-a-valid-timestamp'))).toBe(true);
    expect(result.skippedCount).toBeGreaterThanOrEqual(2);
  });

  it('6. never deletes unrelated directories', () => {
    const exportRoot = mkTempDir('venture-unrelated-dir-exports-');
  ...
  it('7. does not delete symbolic links', () => {
    ...
    const exportRoot = mkTempDir('venture-symlink-exports-');
  ...
  it('8. keeps eligible exports inside the retention window', () => {
    const exportRoot = mkTempDir('venture-inside-retention-');
  ...
  it('9. deletes expired eligible exports', () => {
    const exportRoot = mkTempDir('venture-expired-exports-');
  ...
  it('10. preserves the newest eligible export even when expired', () => {
    const exportRoot = mkTempDir('venture-preserve-newest-');
  ...
  it('11. preserves a single expired eligible export', () => {
    const exportRoot = mkTempDir('venture-single-expired-');
  ...
  it('12. preserves only the newest export and deletes other expired ones', () => {
    const exportRoot = mkTempDir('venture-multi-expired-');
  ...
  it('13. uses deterministic filename ordering when export timestamps tie', () => {
    const exportRoot = mkTempDir('venture-tie-order-');
    const tiedTime = Date.parse('2026-06-01T00:00:00.000Z');
    const dirA = path.join(exportRoot, 'venture-diagnostics-2026-13-40T00-00-00-000Z');
    const dirB = path.join(exportRoot, 'venture-diagnostics-2026-13-39T00-00-00-000Z');
    ...
    const fsImpl: DiagnosticExportRetentionFs = {
      ...
      lstatSync: (p) => {
        ...
        if (base.startsWith('venture-diagnostics-2026-13-')) {
          return {
  ...
  it('14. preserves exports with invalid timestamps conservatively', () => {
    const exportRoot = mkTempDir('venture-invalid-ts-');
    const invalidDir = path.join(exportRoot, 'venture-diagnostics-2026-13-40T00-00-00-000Z');
  ...
  it('15. continues deleting other exports when one deletion fails', () => {
    const exportRoot = mkTempDir('venture-delete-failure-');
    createExportBundle(exportRoot, '2026-05-01T00:00:00.000Z');
  ...
  it('21. uses the existing diagnosticsRetentionDays setting through cleanup options', () => {
    const exportRoot = mkTempDir('venture-retention-days-');
  ...
  it('22. respects minimum and maximum supported retention values', () => {
    const exportRoot = mkTempDir('venture-retention-bounds-');
  ...
  it('23. returns accurate cleanup result counts', () => {
    const exportRoot = mkTempDir('venture-counts-');
  ...
  it('24. logs only aggregate cleanup metadata without sensitive paths', () => {
    const exportRoot = mkTempDir('venture-log-privacy-');
    createExportBundle(exportRoot, '2026-05-01T00:00:00.000Z');
    createExportBundle(exportRoot, '2026-07-01T00:00:00.000Z');
