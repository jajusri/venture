...
describe('release provenance', () => {
  it('accepts clean start and only allowlisted dist changes after build', () => {
    ...
    const end = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => [
        WORKTREE_MODIFIED('apps/venture_desktop/dist/main/main.js'),
        WORKTREE_MODIFIED('apps/venture_desktop/dist/preload/preload.js'),
      ].join('\n'),
    }));
    const generated = validatePostBuildProvenance(start, end);
    expect(generated).toEqual([
      'apps/venture_desktop/dist/main/main.js',
      'apps/venture_desktop/dist/preload/preload.js',
    ]);
  ...
  it('rejects dirty source file at start', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => WORKTREE_MODIFIED('apps/venture_desktop/src/main/main.ts'),
    }));
  ...
  it('rejects untracked source file at start', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => UNTRACKED('apps/venture_desktop/src/new-file.ts'),
    }));
    expect(() => assertReleaseStartClean(start)).toThrow(/dirty at pipeline start/);
  });
  ...
  it('rejects staged change at start', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => STAGED('apps/venture_desktop/package.json'),
    }));
  ...
  it('rejects HEAD changes during build', () => {
    ...
    const end = captureReleaseProvenanceSnapshot(gitRunner({
      ...
      statusPorcelain: () => WORKTREE_MODIFIED('apps/venture_desktop/dist/main/main.js'),
    }));
  ...
  it('rejects non-dist tracked change during build', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({ statusPorcelain: () => '' }));
    const end = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => WORKTREE_MODIFIED('connector/venture_connector/src/bootstrap/app.ts'),
    }));
  ...
  it('rejects staged change after build', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({ statusPorcelain: () => '' }));
    const end = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => STAGED('apps/venture_desktop/dist/main/main.js'),
    }));
  ...
  it('reports allowed dist changes explicitly', () => {
    const start = captureReleaseProvenanceSnapshot(gitRunner({ statusPorcelain: () => '' }));
    const end = captureReleaseProvenanceSnapshot(gitRunner({
      statusPorcelain: () => UNTRACKED('apps/venture_desktop/dist/application/release/build-info.js'),
    }));
    const generated = validatePostBuildProvenance(start, end);
    expect(generated).toEqual(['apps/venture_desktop/dist/application/release/build-info.js']);
  });
  ...
  it('rejects path-prefix lookalike bypass', () => {
    expect(isAllowlistedGeneratedPath('apps/venture_desktop/dist_evil/main.js')).toBe(false);
    expect(isAllowlistedGeneratedPath('apps/venture_desktop/dist/main.js')).toBe(true);
  ...
  it('handles Windows path separators safely', () => {
    expect(normalizeRepoPath('apps\\venture_desktop\\dist\\main\\main.js'))
      .toBe('apps/venture_desktop/dist/main/main.js');
    expect(isAllowlistedGeneratedPath('apps\\venture_desktop\\dist\\main\\main.js')).toBe(true);
  });
