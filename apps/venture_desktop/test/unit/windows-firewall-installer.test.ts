...
describe('Windows installer firewall contract', () => {
  ...
  it('replaces stable-name rules on install and removes them on uninstall', () => {
    expect((script.match(/firewall delete rule name="Venture Connector HTTP"/g) ?? [])).toHaveLength(2);
    expect((script.match(/firewall delete rule name="Venture Connector HTTPS"/g) ?? [])).toHaveLength(2);
    expect((script.match(/firewall delete rule name="Venture Connector mDNS"/g) ?? [])).toHaveLength(2);
  ...
  it('migrates the real legacy transport identity before install-tree replacement', () => {
    ...
    expect(script).toContain('$td018UserAppData\\@venture\\desktop\\connector-transport-identity');
  ...
  it('requires a real over-install assertion before any application launch', () => {
    ...
    expect(lifecycle).toContain('VENTURE_LIFECYCLE_OLD_INSTALLER');
    expect(lifecycle).toContain('VENTURE_LIFECYCLE_OLD_INSTALL_SCOPE');
