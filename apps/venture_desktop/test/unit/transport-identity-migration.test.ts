...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'venture-transport-identity-migration-test-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('migrateLegacyTransportIdentity', () => {
  it('derives the real packaged legacy layout beside dist/main.js', () => {
    expect(resolvePackagedLegacyTransportIdentityDir(
      'C:\\Program Files\\Venture Desktop\\resources\\connector\\dist\\main.js',
    )).toBe(path.normalize('C:\\Program Files\\Venture Desktop\\resources\\connector\\dist\\data\\transport'));
  });
