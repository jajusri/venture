...
class PartyXmlExportScreenTest {
    ...
    private fun candidate(
        ...
        label: String = "GSTIN",
        budcomValue: String? = "29ABCDE1234F1Z5",
        tallyValue: String? = null,
        state: FieldProvenanceState = FieldProvenanceState.BudcomOnlyPending,
    ) = TallyFieldExportCandidate(fieldName, label, tallyValue, budcomValue, state)

    @Test
    fun loadingState() {
        composeRule.setContent {
            BudcomTheme { PartyXmlExportScreen(state = PartyXmlExportUiState(isLoading = true), onEvent = {}) }
        }
    ...
    fun checkTallyIsDisabledWhenNothingIsExportedYet() {
        composeRule.setContent {
            BudcomTheme {
                PartyXmlExportScreen(
                    state = PartyXmlExportUiState(isLoading = false, candidates = listOf(candidate(state = FieldProvenanceState.BudcomOnlyPending))),
                    onEvent = {},
                )
            }
        }
            ...
            BudcomTheme {
                PartyXmlExportScreen(
                    state = PartyXmlExportUiState(
                        isLoading = false,
                        candidates = listOf(candidate()),
                        history = listOf(PartyExportEvent("co-1", "exp-1", "party-1", 0L, "BUDCOM-Tally-Export-ABC.xml", listOf(PartyFieldNames.GSTIN))),
                    ),
                    onEvent = {},
                )
            }
