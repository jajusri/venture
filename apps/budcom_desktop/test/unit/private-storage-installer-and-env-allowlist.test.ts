...
describe('CONNECTOR_CHILD_ENV_ALLOWLIST — private-storage env vars must reach the spawned Connector', () => {
  it('includes both private-storage vault env vars', () => {
    expect(CONNECTOR_CHILD_ENV_ALLOWLIST).toContain('BUDCOM_PRIVATE_STORAGE_VAULT_ID');
    expect(CONNECTOR_CHILD_ENV_ALLOWLIST).toContain('BUDCOM_PRIVATE_STORAGE_MARKER_PATH');
  });

  it('still includes the pre-existing BUDCOM_DATABASE_PATH var (private storage reuses it, does not replace it)', () => {
    expect(CONNECTOR_CHILD_ENV_ALLOWLIST).toContain('BUDCOM_DATABASE_PATH');
  });
});
