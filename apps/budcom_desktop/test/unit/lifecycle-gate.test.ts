...
describe('controlled-pilot lifecycle gate', () => {
  ...
  it('9. rejects mutable data inside install directory', () => {
    expect(() => validateMutableDataOutsideInstall({
      userDataRoot: 'C:\\Users\\Tester\\AppData\\Roaming\\budcom-desktop',
      installRoot: 'C:\\Users\\Tester\\AppData\\Roaming\\budcom-desktop',
    })).toThrow(/must not be inside install root/);
  });

  it('10. detects unexpected startup entries', () => {
    expect(() => detectUnexpectedStartupEntries(['HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Budcom']))
