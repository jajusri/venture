...
describe('artifact classification safety', () => {
  it('classifies NSIS installer filenames', () => {
    expect(classifyArtifactFilename('BudcomDesktop-0.4.3-x64-setup.exe'))
      .toBe(ArtifactClassification.NsisInstaller);
  });

  it('classifies portable zip separately from installer', () => {
    expect(classifyArtifactFilename('BudcomDesktop-0.4.3-x64-portable.zip'))
  ...
  it('rejects placeholder impersonating installer filename', () => {
    expect(() => assertNotPlaceholderInstaller({
      filename: 'BudcomDesktop-0.4.3-x64-setup.exe',
  ...
  it('requires real installer size for NSIS artifacts', () => {
    expect(() => assertRealInstallerArtifact('BudcomDesktop-0.4.3-x64-setup.exe', 1024))
      .toThrow(/too small/);
    expect(() => assertRealInstallerArtifact('BudcomDesktop-0.4.3-x64-setup.exe', minimumInstallerByteSize()))
      .not.toThrow();
  });
  ...
  it('never accepts placeholder as distributable installer', () => {
    expect(isControlledPilotInstallerFilename('BudcomDesktop-0.4.3-x64-setup.exe')).toBe(true);
    const result = evaluateControlledPilotAcceptance({
      platform: 'win32',
      artifacts: [{
        filename: 'BudcomDesktop-0.4.3-x64-setup.exe',
