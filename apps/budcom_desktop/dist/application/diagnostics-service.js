...
class DiagnosticsService {
    ...
    updateResolvedConfig(resolvedConfig) {
    ...
    async exportBundle(targetDir) {
        try {
            const snapshot = await this.getSnapshot();
            const timestamp = snapshot.generatedAt.replace(/[:.]/g, '-');
            const bundleDir = targetDir ?? node_path_1.default.join(this.exportDir, `budcom-diagnostics-${timestamp}`);
            ...
            const bundle = {
                ...
                configuration: (0, log_redaction_js_1.sanitizeConfigForExport)({
                    effective: this.resolvedConfig.effective,
                    sources: this.resolvedConfig.sources,
                    status: snapshot.configStatus,
                }),
                environment: (0, log_redaction_js_1.stripEnvironmentVariables)(process.env),
                logs: {
            ...
            const bundlePath = node_path_1.default.join(bundleDir, 'diagnostics-bundle.json');
