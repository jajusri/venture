...
async function run() {
    ...
    const dashboard = new dashboard_service_js_1.DashboardService({ connectorBaseUrl: CONNECTOR_URL, logService, maxAttempts: 1 });
    const company = new company_service_js_1.CompanyService({ connectorBaseUrl: CONNECTOR_URL, logService, maxAttempts: 1 });
    const results = [];
    ...
    record('S1 Connector Health', healthState.connectorReachable && healthState.connectionLabel === 'Connected' && healthState.healthStatus === 'ok', `reachable=${healthState.connectorReachable} label=${healthState.connectionLabel} health=${healthState.healthStatus}`);
    ...
    record('S2 Company Discovery', companies.status === 'SUCCESS' && companies.items.length > 0 && Boolean(liveCompany), `status=${companies.status} count=${companies.items.length} company=${liveCompany?.name ?? 'none'} id=${liveCompany?.id ?? 'none'}`);
    if (!liveCompany) {
    ...
    record('S3 Company Selection', selection.ok &&
        ...
        afterSelect.lastRefresh !== '—', `selection=${selection.status} company=${afterSelect.companyName} session=${afterSelect.sessionStatus}`);
    // Scenario 4 — Session Validation (authoritative connector session)
    const afterValidate = await dashboard.getDashboardState();
    record('S4 Session Validation', afterValidate.sessionStatus === 'ACTIVE' && afterValidate.companyId === liveCompany.id, `session=${afterValidate.sessionStatus} companyId=${afterValidate.companyId}`);
    // Scenario 5 — Clear Selection
    await company.clearSelection();
    const afterClear = await dashboard.getDashboardState();
    record('S5 Clear Selection', afterClear.sessionStatus === 'NO_COMPANY_SELECTED' && afterClear.companyName === '—', `session=${afterClear.sessionStatus} company=${afterClear.companyName}`);
    // Scenario 6 — Reselect
    const reselect = await company.selectCompany(liveCompany.id);
    const afterReselect = await dashboard.getDashboardState();
    record('S6 Reselect Company', reselect.ok && afterReselect.sessionStatus === 'ACTIVE' && afterReselect.companyId === liveCompany.id, `selection=${reselect.status} session=${afterReselect.sessionStatus}`);
    // Scenario 7 — Desktop Restart (simulated: new service instances, same connector session)
    const restartDashboard = new dashboard_service_js_1.DashboardService({ connectorBaseUrl: CONNECTOR_URL, maxAttempts: 1 });
    const afterRestart = await restartDashboard.getDashboardState();
    record('S7 Desktop Restart', afterRestart.companyId === liveCompany.id && afterRestart.sessionStatus === 'ACTIVE', `persisted company=${afterRestart.companyName} session=${afterRestart.sessionStatus} (connector in-memory session)`);
