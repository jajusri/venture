...
describe('relay HTTP submission', () => {
  it('accepts an authenticated envelope once and returns the same acceptance on retry', async () => {
    ...
    const app = buildRelayService({ repository, verifier, mailboxVerifier, issuer: issuer(), now: () => new Date(10) }); apps.push(app);
  ...
  it('rejects forged authority without creating a mailbox entry', async () => {
    const repository = new MemoryRepository();
    const app = buildRelayService({ repository, verifier: { verify: async (value) => ({ ...(await verifier.verify(value)), credentialValid: false }) }, mailboxVerifier, issuer: issuer() });
    apps.push(app);
  ...
  it('returns only the authenticated recipient mailbox page', async () => {
    ...
    const app = buildRelayService({ repository, verifier, mailboxVerifier, issuer: issuer() }); apps.push(app);
