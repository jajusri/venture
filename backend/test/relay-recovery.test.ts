...
const submission = (): RelaySubmission => ({
  ...
  authenticatedEnvelope: new Uint8Array([1]), commercialContent: '{}', commercialContentType: 'application/vnd.venture.order-snapshot+json', commercialContentVersion: 3,
...
const verified = (): Awaited<ReturnType<RelaySubmissionVerifier['verify']>> => ({
  ...
  commercialContent: '{}', commercialContentType: 'application/vnd.venture.order-snapshot+json', commercialContentVersion: 3,
});
...
describe('relay failure and recovery attacks', () => {
  ...
  it('absorbs retry storms with bounded overload while preserving idempotent acceptance', async () => {
    ...
    const body = {
      ...
      commercialContentType: 'application/vnd.venture.order-snapshot+json', commercialContentVersion: 3, submittedAt: new Date(1).toISOString(),
    };
