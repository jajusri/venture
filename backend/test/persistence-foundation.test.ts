...
describe('PostgreSQL persistence foundation', () => {
  ...
  it('runs additive migrations and parameterizes registry writes', async () => {
    ...
    expect(registryWrites.map((call) => call.parameters)).toEqual([[1, 'migration_registry'], [2, 'trust_authority_state'], [3, 'relay_durable_mailbox'], [4, 'relay_acceptance_evidence'], [5, 'relay_authenticated_commercial_content'], [6, 'relay_authenticated_request_replay_guard']]);
  });
});
