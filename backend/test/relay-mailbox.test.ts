...
import { encodeRelayCursor, relayIdentifier, type RelayAcceptance, type RelayMailboxEntry, type RelaySubmission } from '../services/relay/src/domain/relay.js';
...
describe('relay HTTP mailbox fetch', () => {
  it('returns only the authenticated recipient mailbox page', async () => {
    const repository = new MemoryRepository();
    repository.entries = [entry(1)];
    const app = buildRelayService({ repository, verifier: submissionVerifier, mailboxVerifier: verifier, issuer: issuer() });
    apps.push(app);
